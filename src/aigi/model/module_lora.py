...
class LoRALinearAdapter:
    def __init__(self, base: Any, *, rank: int, alpha: float) -> None:
        ...
        self.module.base = base
        self.module.rank = rank
        self.module.alpha = alpha
        self.module.scale = alpha / rank
        for parameter in base.parameters():
            parameter.requires_grad_(False)
        self.module.lora_a = torch.nn.Parameter(
            torch.randn(base.in_features, rank, device=base.weight.device, dtype=torch.float32) * 0.01
        )
        self.module.lora_b = torch.nn.Parameter(
            torch.zeros(rank, base.out_features, device=base.weight.device, dtype=torch.float32)
        )

        def forward(inputs: Any) -> Any:
            base_output = self.module.base(inputs)
            delta = (inputs.float() @ self.module.lora_a @ self.module.lora_b) * self.module.scale
    ...
    def parameters(self) -> list[Any]:
        return [self.module.lora_a, self.module.lora_b]
...
class ModuleLoRAAdapterTrainer:
    ...
    def train(
        ...
        *,
        ...
        setattr(parent, attribute, adapter.module)
        ...
        torch.save(
            {
                ...
                "rank": rank,
                "alpha": alpha,
                "lora_a": adapter.module.lora_a.detach().cpu(),
                "lora_b": adapter.module.lora_b.detach().cpu(),

...
encode → LoRA train (steps=400, strong edit) → merge → re-encode → eval
Reload model fresh for each K to avoid OOM.
"""
...
# ── Main ─────────────────────────────────────────────────
def load_fresh_model(tokenizer, device):
    model = AutoModelForCausalLM.from_pretrained(MODEL_ID, torch_dtype=torch.bfloat16)
    return model.to(device)

def test_k(K, tokenizer, device, baseline_ppl, baseline_surv):
    print(f"\n{'='*50}", flush=True)
    print(f"Testing K={K}", flush=True)
    print(f"{'='*50}", flush=True)
    
    model = load_fresh_model(tokenizer, device)
    ...
    enc_surv = eval_survival(model, tokenizer, device, EDIT_FACTS)
    print(f"  Encoded: PPL={enc_ppl:.4f}, Survival={enc_surv}/10, Time={enc_time:.1f}s", flush=True)
    
    # Train LoRA
    t0 = time.time()
    model = train_lora(model, tokenizer, EDIT_FACTS, steps=STEPS, rank=RANK,
                      target_layers=TARGET_LAYERS, target_modules=TARGET_MODULES,
                      lr=LR, device=device)
    train_time = time.time() - t0
    lora_ppl = eval_ppl(model, tokenizer, device)
    lora_surv = eval_survival(model, tokenizer, device, EDIT_FACTS)
    ...
    merge_surv = eval_survival(model, tokenizer, device, EDIT_FACTS)
    ...
    reenc_surv = eval_survival(model, tokenizer, device, EDIT_FACTS)
    print(f"  After Re-enc: PPL={reenc_ppl:.4f}, Survival={reenc_surv}/10, Time={reenc_time:.1f}s", flush=True)
    ...
    return {
        "K": K,
        "encoded_ppl": enc_ppl, "encoded_survival": enc_surv, "encode_time": enc_time,
        "lora_ppl": lora_ppl, "lora_survival": lora_surv, "train_time": train_time,
        "merge_ppl": merge_ppl, "merge_survival": merge_surv,
        "reencode_ppl": reenc_ppl, "reencode_survival": reenc_surv, "reencode_time": reenc_time,
    }

def main():
    print("=" * 60, flush=True)
    print("M213 — K Sweep for Compiled Edits", flush=True)
    print("=" * 60, flush=True)
    ...
    # Baseline (single load, then free)
    print(f"Loading {MODEL_ID} for baseline...", flush=True)
    model = load_fresh_model(tokenizer, device)
    baseline_ppl = eval_ppl(model, tokenizer, device)
    baseline_surv = eval_survival(model, tokenizer, device, EDIT_FACTS)
    print(f"Baseline: PPL={baseline_ppl:.4f}, Survival={baseline_surv}/10\n", flush=True)
    del model
    gc.collect()
    torch.cuda.empty_cache()
    
    # Test each K (reload fresh model each time)
    results = []
    for K in K_VALUES:
        r = test_k(K, tokenizer, device, baseline_ppl, baseline_surv)
    ...
    print(f"{'K':>5} {'Enc Δ':>10} {'LoRA Δ':>10} {'Merge Δ':>10} {'Re-enc Δ':>11} {'Survival':>10} {'EncT':>8} {'ReT':>8}", flush=True)
    print("-" * 85, flush=True)
    for r in results:
        print(f"{r['K']:>5} {r['encoded_ppl']-r['baseline_ppl']:>+10.4f} "
              f"{r['lora_ppl']-r['baseline_ppl']:>+10.4f} {r['merge_ppl']-r['baseline_ppl']:>+10.4f} "
              f"{r['reencode_ppl']-r['baseline_ppl']:>+11.4f} {r['reencode_survival']:>10.0f}/10 "
              f"{r['encode_time']:>7.1f}s {r['reencode_time']:>7.1f}s", flush=True)
    
    with open("experiments/m213_results.json", "w") as f:
        json.dump(results, f, indent=2)
    print("\n✅ Saved to experiments/m213_results.json", flush=True)

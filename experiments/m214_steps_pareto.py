...
Build survival vs PPL curve to find optimal steps/Pareto frontier.
"""
...
STEPS_LIST = [50, 100, 200, 300, 400, 600, 800]

from experiments.facts_50 import FACTS_50
EDIT_FACTS = FACTS_50[:10]

# ── Encode helpers (same as M213) ────────────────────────
...
# ── Main ─────────────────────────────────────────────────
def test_steps(steps, model_base, tokenizer, device):
    print(f"\n{'='*50}", flush=True)
...
def main():
    print("=" * 60, flush=True)
    print("M214 — Steps Pareto Frontier", flush=True)
    print("=" * 60, flush=True)
    ...
    baseline_surv = eval_survival(model_base, tokenizer, device, EDIT_FACTS)
    print(f"Baseline: PPL={baseline_ppl:.4f}, Survival={baseline_surv}/10\n", flush=True)
    
    # Test each steps
    results = []
    for steps in STEPS_LIST:
        r = test_steps(steps, model_base, tokenizer, device)
    ...
    # Summary
    print(f"\n{'='*60}", flush=True)
    print("SUMMARY — Steps Pareto Frontier", flush=True)
    print(f"{'='*60}", flush=True)
    print(f"{'Steps':>7} {'LoRA Δ':>10} {'Merge Δ':>10} {'Re-enc Δ':>11} {'Survival':>10} {'Time':>8}", flush=True)
    print("-" * 75, flush=True)
    for r in results:
        print(f"{r['steps']:>7} {r['lora_ppl']-r['baseline_ppl']:>+10.4f} "
              f"{r['merge_ppl']-r['baseline_ppl']:>+10.4f} {r['reencode_ppl']-r['baseline_ppl']:>+11.4f} "
              f"{r['reencode_survival']:>10.0f}/10 {r['train_time']:>7.1f}s", flush=True)
    
    # Find Pareto-optimal points
    print(f"\n{'='*60}", flush=True)
    print("PARETO-OPTIMAL POINTS (max survival, min ΔPPL)", flush=True)
    print(f"{'='*60}", flush=True)
    points = [(r['steps'], r['reencode_survival'], r['reencode_ppl']-r['baseline_ppl']) for r in results]
    for i, (s, surv, dppl) in enumerate(points):
        is_pareto = True
        for j, (s2, surv2, dppl2) in enumerate(points):
            if i != j and surv2 >= surv and dppl2 <= dppl and (surv2 > surv or dppl2 < dppl):
                is_pareto = False
                break
        if is_pareto:
            print(f"  steps={s}: survival={surv}/10, ΔPPL={dppl:+.4f} ✅ PARETO", flush=True)
    
    with open("experiments/m214_results.json", "w") as f:
        json.dump(results, f, indent=2)
    print("\n✅ Saved to experiments/m214_results.json", flush=True)

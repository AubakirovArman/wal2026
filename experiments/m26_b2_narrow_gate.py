...
TARGETS = ("model.language_model.layers.54.mlp.gate_proj", "model.language_model.layers.54.mlp.up_proj")
...
def _hot_hit_stats(mod: PackedGroupedBlockRVQLinear, args: argparse.Namespace) -> tuple[float, float]:
    ...
    for group in mod.groups:
        group._build_hot_cache(torch.bfloat16, group.row_scale.device, int(args.hot_topk), args.hot_score_mode, float(args.hot_min_stage_share))
...
def _run_target(name: str, enc, args: argparse.Namespace, device: torch.device) -> dict[str, float | bool | str]:
    ...
    test_mod = _make_module(enc, "stage_local_hot_cold_b2", args, device)
    ...
    result["pass_narrow_gate"] = bool(
        ...
        and speedup >= 1.2
        and hit_min >= float(args.min_hot_hit_rate)
    )
    return result
...
def main() -> None:
    ...
    ap.add_argument("--out", default=str(ROOT / "dwl2_dynamic_route/results/m26_b2_narrow_gate.json"))
    ap.add_argument("--device", default="cuda")
    ap.add_argument("--m", type=int, default=2048)
    ap.add_argument("--warmup", type=int, default=20)
    ap.add_argument("--iters", type=int, default=50)
    ap.add_argument("--onehot-chunk", type=int, default=256)
    ap.add_argument("--hot-topk", type=int, default=16)
    ap.add_argument("--hot-score-mode", choices=("count", "row_scale_norm"), default="row_scale_norm")
    ap.add_argument("--hot-min-stage-share", type=float, default=0.0)
    ap.add_argument("--min-hot-hit-rate", type=float, default=0.72)
    ...
    for name, item in results.items():
        ...
        print(
            ...
            f"hot_v2={item['ms_hot_v2']:.3f}ms b2={item['ms_b2']:.3f}ms speedup={item['speedup_vs_hot_v2']:.3f} "

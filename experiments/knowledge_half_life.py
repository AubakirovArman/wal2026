"""
Wild Idea #5 — Knowledge Half-Life

Each edit has a half-life. Auto-refresh scheduler.
"""
...
def measure_half_life(model, tokenizer, facts, new_facts):
    """Measure how many new edits until old facts degrade."""
    survival_history = []
    for i, new_fact in enumerate(new_facts):
        model = train_lora_fp32(model, tokenizer, [new_fact], SEED + i)
        # Test old facts
        survived = sum(1 for q, a in facts if a.lower() in generate_answer(model, tokenizer, q).lower())
        survival_history.append({"edit": i + 1, "old_survival": survived, "total_old": len(facts)})
        if survived == 0:
            break
    return survival_history

print("=" * 60)
print("KNOWLEDGE HALF-LIFE")
print("=" * 60)
...
# Measure half-life with new edits
new_facts = [
...
history = measure_half_life(model, tokenizer, FACTS, new_facts)

for h in history:
    print(f"  After edit {h['edit']}: old survival {h['old_survival']}/{h['total_old']}")

if len(history) < len(new_facts):
    hl = history[-1]["edit"]
    print(f"\n🎯 Half-life: ~{hl} edits (old facts lost after {hl} new edits)")
else:
    print(f"\n🎯 Half-life: >{len(new_facts)} edits (old facts still survive)")

with open("experiments/knowledge_half_life_results.json", "w") as f:

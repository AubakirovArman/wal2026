...
Base_v0 → Edit_1 → merge/re-encode → Base_v1 → Edit_2 → ... → Base_v10

Track at each step:
- PPL
- Survival of current batch
- Survival of ALL previous batches (drift test)
"""
...
def main():
    print("=" * 60, flush=True)
    print("M215 — Sequential Long Chain (10 edits)", flush=True)
    print("=" * 60, flush=True)
    ...
    print(f"Base WAL: PPL={base_ppl:.4f} (Δ={base_ppl-baseline_ppl:+.4f})", flush=True)
    ...
    for edit_idx in range(N_EDITS):
        ...
        print(f"\n{'='*50}", flush=True)
        print(f"Edit {edit_idx+1}/{N_EDITS}: {len(batch)} facts", flush=True)
        print(f"{'='*50}", flush=True)
        
        # Train LoRA on this batch
        t0 = time.time()
        ...
        lora_batch_surv = eval_survival(model, tokenizer, device, batch)
        lora_cumul_surv = eval_survival(model, tokenizer, device, cumulative_facts)
        print(f"  After LoRA: PPL={lora_ppl:.4f} (Δ={lora_ppl-baseline_ppl:+.4f}), "
              f"Batch={lora_batch_surv}/{len(batch)}, Cumulative={lora_cumul_surv}/{len(cumulative_facts)}, "
              f"Time={train_time:.1f}s", flush=True)
        
        # Merge
        model = merge_lora(model)
        merge_ppl = eval_ppl(model, tokenizer, device)
        merge_batch_surv = eval_survival(model, tokenizer, device, batch)
        merge_cumul_surv = eval_survival(model, tokenizer, device, cumulative_facts)
        print(f"  After Merge: PPL={merge_ppl:.4f} (Δ={merge_ppl-baseline_ppl:+.4f}), "
              f"Batch={merge_batch_surv}/{len(batch)}, Cumulative={merge_cumul_surv}/{len(cumulative_facts)}", flush=True)
        ...
        reenc_batch_surv = eval_survival(model, tokenizer, device, batch)
        reenc_cumul_surv = eval_survival(model, tokenizer, device, cumulative_facts)
        print(f"  After Re-enc: PPL={reenc_ppl:.4f} (Δ={reenc_ppl-baseline_ppl:+.4f}), "
              f"Batch={reenc_batch_surv}/{len(batch)}, Cumulative={reenc_cumul_surv}/{len(cumulative_facts)}, "
              f"Time={reenc_time:.1f}s", flush=True)
        
        results.append({
            "edit": edit_idx + 1,
            "batch_size": len(batch),
            "lora_ppl": lora_ppl, "lora_batch_survival": lora_batch_surv, "lora_cumulative_survival": lora_cumul_surv,
            "merge_ppl": merge_ppl, "merge_batch_survival": merge_batch_surv, "merge_cumulative_survival": merge_cumul_surv,
            "reencode_ppl": reenc_ppl, "reencode_batch_survival": reenc_batch_surv, "reencode_cumulative_survival": reenc_cumul_surv,
            "train_time": train_time, "reencode_time": reenc_time,
        })
    
    # Summary
    print(f"\n{'='*60}", flush=True)
    print("SUMMARY — Sequential Long Chain", flush=True)
    print(f"{'='*60}", flush=True)
    print(f"{'Edit':>5} {'Re-enc PPL':>12} {'Δ PPL':>10} {'Batch Surv':>12} {'Cumul Surv':>12}", flush=True)
    print("-" * 60, flush=True)
    for r in results:
        print(f"{r['edit']:>5} {r['reencode_ppl']:>12.4f} {r['reencode_ppl']-baseline_ppl:>+10.4f} "
              f"{r['reencode_batch_survival']:>11.0f}/{r['batch_size']} {r['reencode_cumulative_survival']:>11.0f}/{len(cumulative_facts)}", flush=True)
    
    # Drift analysis
    print(f"\n{'='*60}", flush=True)
    print("DRIFT ANALYSIS", flush=True)
    print(f"{'='*60}", flush=True)
    
    # Early batch survival at final step
    early_batches = [batches[0], batches[1], batches[2]]
    for idx, batch in enumerate(early_batches):
        surv = eval_survival(model, tokenizer, device, batch)
        print(f"  Batch {idx+1} survival at final Base_v{N_EDITS}: {surv}/{len(batch)}", flush=True)
    
    final_result = {
        "baseline_ppl": baseline_ppl,
        "base_wal_ppl": base_ppl,
        "edits": results,
        "final_cumulative_survival": reenc_cumul_surv,
        "final_total_facts": len(cumulative_facts),
    }
    
    with open("experiments/m215_results.json", "w") as f:
        json.dump(final_result, f, indent=2)
    print("\n✅ Saved to experiments/m215_results.json", flush=True)

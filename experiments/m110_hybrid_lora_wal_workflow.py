#!/usr/bin/env python3
"""M110 / Phase 15: Hybrid LoRA→WAL Workflow

End-to-end proof that the cycle works:
    Dense → WAL → Dense → LoRA Edit → Merge → WAL

Verifies that post-re-encode model preserves both edited behavior
and base model quality (PPL).

This implements the path forward from M100-M101 conclusions:
    "Edit in weight space, store in WAL space."

NOTE: Uses manual training loop (not transformers.Trainer) because
Trainer adds hooks/wrappers that interfere with replace_linear_with_wal.
"""
import torch
...
def inject_lora(model, target_layers):
    """Inject LoRA into o_proj of target layers. Returns original forwards dict."""
    original_forwards = {}
...
def merge_lora(model, target_layers, original_forwards):
    """Merge LoRA deltas into base weights and restore original forward."""
    for i in target_layers:
        layer = model.model.layers[i].self_attn.o_proj
        with torch.no_grad():
...
def replace_wal_with_dense(model):
    """Replace WALLinear / WALCachedLinear with nn.Linear using decoded weights."""
...
def main():
    print("=" * 70)
    print("M110 / Phase 15: Hybrid LoRA→WAL Workflow")
    print("=" * 70)
    ...
    # ------------------------------------------------------------------
    # 2. Dense baseline: PPL + contrafactuals
    # ------------------------------------------------------------------
    ...
    _, dense_acc = evaluate_facts(model, tokenizer, "dense baseline")

    # ------------------------------------------------------------------
    # 3. Encode ALL linear layers to WAL (cached for speed)
    # ------------------------------------------------------------------
    print("\n[3] Encoding dense → WAL (all layers, cached=True)...", flush=True)
    t0 = time.time()
    replace_linear_with_wal(
        model, K=256, C=16, build_hier=False, cached=True
    )
    encode_time = time.time() - t0
    print(f"    Encode time: {encode_time:.1f}s", flush=True)

    # ------------------------------------------------------------------
    # 4. WAL baseline: PPL + contrafactuals
    # ------------------------------------------------------------------
    print("\n[4] WAL-encoded baseline evaluation...", flush=True)
    wal_ppl = compute_ppl(model, tokenizer, wikitext_texts)
    print(f"    PPL: {wal_ppl:.4f} (Δ vs dense: {wal_ppl - dense_ppl:+.4f})", flush=True)
    _, wal_acc = evaluate_facts(model, tokenizer, "WAL-encoded")
    ...
    print(f"    Decode time: {decode_time:.1f}s", flush=True)

    # ------------------------------------------------------------------
    # 6. Post-decode dense: PPL + contrafactuals
    # ------------------------------------------------------------------
    print("\n[6] Post-decode dense evaluation...", flush=True)
    decoded_ppl = compute_ppl(model, tokenizer, wikitext_texts)
    print(f"    PPL: {decoded_ppl:.4f} (Δ vs dense: {decoded_ppl - dense_ppl:+.4f})", flush=True)
    _, decoded_acc = evaluate_facts(model, tokenizer, "post-decode dense")

    # ------------------------------------------------------------------
    # 7. Inject LoRA and train on contrafactuals (manual loop)
    # ------------------------------------------------------------------
    print("\n[7] Injecting LoRA + training on contrafactuals...", flush=True)
    ...
    # ------------------------------------------------------------------
    # 8. Merge LoRA into base weights
    # ------------------------------------------------------------------
    print("\n[8] Merging LoRA into base weights...", flush=True)
    merge_lora(model, TARGET_LAYERS, orig_forwards)

    # Quick check: post-merge dense should already answer contrafactuals
    print("\n[9] Post-merge dense (before re-encode):", flush=True)
    merged_ppl = compute_ppl(model, tokenizer, wikitext_texts)
    print(f"    PPL: {merged_ppl:.4f} (Δ vs dense: {merged_ppl - dense_ppl:+.4f})", flush=True)
    _, merged_acc = evaluate_facts(model, tokenizer, "post-merge dense")

    # ------------------------------------------------------------------
    # 10. Re-encode merged dense → WAL
    # ------------------------------------------------------------------
    print("\n[10] Re-encoding merged dense → WAL...", flush=True)
    t0 = time.time()
    replace_linear_with_wal(
        model, K=256, C=16, build_hier=False, cached=True
    )
    ...
    # ------------------------------------------------------------------
    # 11. Final WAL verification
    # ------------------------------------------------------------------
    print("\n[11] Final WAL model (post-edit + re-encode):", flush=True)
    final_ppl = compute_ppl(model, tokenizer, wikitext_texts)
    print(f"    PPL: {final_ppl:.4f} (Δ vs dense: {final_ppl - dense_ppl:+.4f})", flush=True)
    ...
    # ------------------------------------------------------------------
    print("\n" + "=" * 70)
    print("M110 / Phase 15: SUMMARY")
    print("=" * 70)
    print(f"\n  Perplexity:")
    print(f"    Dense baseline:   {dense_ppl:.4f}")
    print(f"    WAL-encoded:      {wal_ppl:.4f}  (Δ={wal_ppl - dense_ppl:+.4f})")
    print(f"    Post-decode:      {decoded_ppl:.4f}  (Δ={decoded_ppl - dense_ppl:+.4f})")
    print(f"    Post-merge dense: {merged_ppl:.4f}  (Δ={merged_ppl - dense_ppl:+.4f})")
    print(f"    Final WAL:        {final_ppl:.4f}  (Δ={final_ppl - dense_ppl:+.4f})")
    print(f"\n  Contrafactual accuracy:")
    print(f"    Dense baseline:   {dense_acc:.0%}")
    print(f"    WAL-encoded:      {wal_acc:.0%}")
    print(f"    Post-decode:      {decoded_acc:.0%}")
    print(f"    Post-merge dense: {merged_acc:.0%}")
    print(f"    Final WAL:        {final_acc:.0%}")
    print(f"\n  Timing:")
    print(f"    Encode:     {encode_time:.1f}s")
    print(f"    Decode:     {decode_time:.1f}s")
    print(f"    Re-encode:  {reencode_time:.1f}s")

    ok_edit = final_acc >= 0.8
    # Re-encode should not ADD degradation beyond what overfit already caused
    ok_ppl = abs(final_ppl - merged_ppl) < 0.5
    if ok_edit and ok_ppl:
        print("\n  ✅ PASS: Hybrid workflow works! Edits survive re-encoding.")
    elif ok_edit:
        print("\n  🟡 PARTIAL: Edits survive, but re-encoding added PPL degradation.")
    else:
        print("\n  ❌ FAIL: Edits lost after re-encoding.")
    print("=" * 70)

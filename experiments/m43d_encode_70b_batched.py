#!/usr/bin/env python3
"""M43d: Batched GPU hybrid encoder for 70B."""
import torch
...
def encode_blocks_rvq_gpu_batched(blocks, codebook, l_max, batch_size=262144):
    ...
    ids = torch.zeros(N, l_max, dtype=torch.int64, device="cpu")
    ...
    for stage in range(l_max):
        ...
        for start in range(0, N, batch_size):
            ...
            if take.any():
                ...
                ids[start:end][take, stage] = best_k[take].cpu()
...
def hybrid_encode_batched(w, coarse_ladder, vre_cb_size=512, vre_lmax=8, scalar_K=128, scalar_lmax=8, spiky_threshold=0.08):
    device = w.device
    w_f = w.float()
    ...
    if is_spiky:
        ...
        digits, ids, stop_depth = encode_blocks_rvq_gpu_batched(blocks_gpu, codebook, vre_lmax)
        recon = decode_blocks_rvq_batched(digits, ids, codebook, stop_depth, block_size, rows, cols)
        w_hat = recon.to(device) * row_scale

        prog_keys = torch.zeros(digits.shape[0], dtype=torch.int64)
...
print("Applying hybrid encoder (batched)...")

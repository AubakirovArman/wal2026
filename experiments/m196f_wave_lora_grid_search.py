#!/usr/bin/env python3
"""M196f — Wave-LoRA Grid Search λ with n=20 runs.

Statistical confidence test for optimal λ on mixed targets.
Grid: [0.01, 0.015, 0.02, 0.025, 0.03]
"""
...
def main():
    print("=" * 60, flush=True)
    print("M196f — Wave-LoRA Grid Search λ (n=20 runs)", flush=True)
    print("=" * 60, flush=True)
    device = DEVICE
    ...
    n_runs = 20
    all_results = {str(l): [] for l in lambdas}
    ...
    print("\n" + "=" * 60, flush=True)
    print("Summary (n=20 runs)", flush=True)
    print("=" * 60, flush=True)
    print(f"{'λ':>8} {'Mean':>8} {'Std':>8} {'Min':>6} {'Max':>6}", flush=True)
    print("-" * 50, flush=True)
    import statistics
    for wave_lambda in lambdas:
        vals = all_results[str(wave_lambda)]
        mean = statistics.mean(vals)
        std = statistics.stdev(vals) if len(vals) > 1 else 0
        print(f"{wave_lambda:>8.4f} {mean:>8.2f} {std:>8.2f} {min(vals):>6} {max(vals):>6}", flush=True)

    with open("/mnt/hf_model_weights/arman/3bit/wal/experiments/m196f_results.json", "w") as f:
        json.dump({"n_runs": n_runs, "results": all_results}, f, indent=2)
    print("\n✅ Saved to experiments/m196f_results.json", flush=True)

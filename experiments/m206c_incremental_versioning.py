...
Each edit is a "version" that can be rolled back to.
"""
...
# Split into 3 groups
FACTS_PER_GROUP = len(FACTS_50) // 3
GROUPS = [
    FACTS_50[0:FACTS_PER_GROUP],
    FACTS_50[FACTS_PER_GROUP:2*FACTS_PER_GROUP],
    FACTS_50[2*FACTS_PER_GROUP:],
]
print(f"Groups: {[len(g) for g in GROUPS]} facts each")

TARGET_LAYERS = [14, 15, 16]
TARGET_MODULES = ["o_proj", "q_proj", "v_proj", "gate_proj"]

# ── Encode helpers (same as M206b) ───────────────────────
...
def main():
    print(f"\n{'='*60}", flush=True)
    print(f"M206c — Incremental Editing with 3 Groups", flush=True)
    ...
    for run in range(3):
        ...
        print(f"Encoded: PPL={enc_ppl:.4f}", flush=True)
        
        versions = []
        cumulative_survs = []
        
        for gi, group_facts in enumerate(GROUPS):
            print(f"\n  === Group {gi+1}/3 ({len(group_facts)} facts) ===", flush=True)
            
            print(f"  Training LoRA...", flush=True)
            t0 = time.time()
            model = train_lora(model, tokenizer, group_facts, steps=STEPS, rank=RANK,
            ...
            # Re-encode
            print(f"  Re-encoding...", flush=True)
            t0 = time.time()
            model = encode_model(model, K=K, iters=ITERS)
            print(f"    Done in {time.time()-t0:.1f}s", flush=True)
            ...
            versions.append({
                "group": gi+1,
                "ppl_after_train": ppl,
                "surv_after_train": surv_all,
                "surv_after_merge": surv_merge,
                "surv_after_reenc": surv_reenc,
            })
            cumulative_survs.append(surv_reenc)
        
        final_ppl = eval_ppl(model, tokenizer, device)
        final_surv = eval_survival(model, tokenizer, device, FACTS_50)
        print(f"\n  FINAL: PPL={final_ppl:.4f}, Survival={final_surv}/50", flush=True)
    ...
    import statistics
    print(f"\n{'='*60}", flush=True)
    print(f"SUMMARY — 3-Group Incremental Editing", flush=True)
    print(f"{'='*60}", flush=True)
    
    for gi in range(3):
        print(f"\n--- After Group {gi+1} ---", flush=True)
        t_survs = [r["versions"][gi]["surv_after_train"] for r in results]
        m_survs = [r["versions"][gi]["surv_after_merge"] for r in results]
        r_survs = [r["versions"][gi]["surv_after_reenc"] for r in results]
        ppls = [r["versions"][gi]["ppl_after_train"] for r in results]
    ...
    with open("experiments/m206c_results.json", "w") as f:
        json.dump(results, f, indent=2)
    print("\n✅ Saved to experiments/m206c_results.json", flush=True)

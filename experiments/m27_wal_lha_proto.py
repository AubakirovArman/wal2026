...
itself. Instead of relying on only a static family-conditioned phrase bank,
each low-level atom becomes a tiny expressive module that produces
context-dependent token logits on top of a learned base phrase prior.
...
For each target layer:
  ...
  - train a semantic encoder against:
      ...
      * a separate atom-selection loss for choosing the best expressive atom
      * a direct atom-usage bonus when expressive atoms beat the static bank
  - measure two surfaces:
      1. exact WAL-LHA: FAMILY + decisioned expressive atoms + literal corrections
      2. lha_only: FAMILY-conditioned expressive atoms only, no corrections
"""
...
class ExpressiveAtomGenerator(nn.Module):
    def __init__(
        self,
        hidden_dim: int,
        atom_hidden_dim: int,
        ...
        self.base = int(base)
        self.context_proj = nn.Linear(hidden_dim, atom_hidden_dim)
        self.family_embed = nn.Embedding(num_families, atom_hidden_dim)
        self.variant_embed = nn.Embedding(num_slot_variants, atom_hidden_dim)
        self.slot_embed = nn.Embedding(num_slots, atom_hidden_dim)
        self.atom_mlp = nn.Sequential(
            nn.Linear(atom_hidden_dim, atom_hidden_dim),
            nn.SiLU(),
            nn.Linear(atom_hidden_dim, phrase_len * base),
        )
        self.num_families = int(num_families)
        ...
        self.base_phrase_logits = nn.Parameter(phrase_logits)
        with torch.no_grad():
            self.atom_mlp[-1].weight.zero_()
            self.atom_mlp[-1].bias.zero_()

    def forward(self, hidden: torch.Tensor, slot_idx: int) -> torch.Tensor:
        context = self.context_proj(hidden).unsqueeze(1).unsqueeze(2)
        family = self.family_embed.weight.unsqueeze(0).unsqueeze(2)
        variant = self.variant_embed.weight.unsqueeze(0).unsqueeze(0)
        slot = self.slot_embed.weight[int(slot_idx)].view(1, 1, 1, -1)
        expressive = self.atom_mlp(context + family + variant + slot).view(
            int(hidden.shape[0]),
            self.num_families,
            self.num_slot_variants,
            self.phrase_len,
            self.base,
        )
        return self.base_phrase_logits[:, int(slot_idx)].unsqueeze(0) + expressive

    def base_tokens(self) -> torch.Tensor:
        return self.base_phrase_logits.argmax(dim=-1).to(torch.uint8)


class HighExpressivityAtomISAEncoder(nn.Module):
    def __init__(
        self,
        input_dim: int,
        hidden_dim: int,
        ...
        self.atom_selector_heads = nn.ModuleList(
        ...
        self.atom_generator = ExpressiveAtomGenerator(
            hidden_dim=hidden_dim,
            atom_hidden_dim=atom_hidden_dim,
            num_families=num_families,
            num_slots=num_slots,
            num_slot_variants=num_slot_variants,
    ...
    def forward(self, features: torch.Tensor) -> tuple[torch.Tensor, list[torch.Tensor], torch.Tensor, torch.Tensor]:
        ...
        atom_logits = [head(hidden).view(-1, self.num_families, self.num_slot_variants) for head in self.atom_selector_heads]
    ...
    def slot_token_logits(self, hidden: torch.Tensor, slot_idx: int) -> torch.Tensor:
        return self.atom_generator(hidden, slot_idx)

    def base_phrase_tokens(self) -> torch.Tensor:
        return self.atom_generator.base_tokens()
...
def _predict_lha_hard_from_features(
    ...
    with torch.no_grad():
        for start in range(0, total_rows, chunk_size):
            ...
            family_logits, atom_logits, decision_logits, hidden = model(norm_features[start:end])
            ...
            for slot_idx in range(model.num_slots):
                ...
                slot_token_logits = model.slot_token_logits(hidden, slot_idx)
                approx_sequences[
                    start:end,
                    slot_idx * model.phrase_len:(slot_idx + 1) * model.phrase_len,
                ] = slot_token_logits[row_idx, hard_family, hard_atom].argmax(dim=-1).to(torch.uint8)
    return family_ids, atom_ids, use_atom, approx_sequences, logit_margin, decision_prob


def _hard_lha_program_cost(
...
def _soft_lha_loss(
    ...
    family_logits, atom_logits, decision_logits, hidden = model(features)
    family_probs = F.softmax(family_logits / float(args.family_temp), dim=1)
    decision_probs = torch.sigmoid(decision_logits)
    base_phrase_tokens = model.base_phrase_tokens().to(torch.int64)
    ...
    phrase_entropy = torch.zeros((), dtype=torch.float32)
    decision_loss = torch.zeros((), dtype=torch.float32)
    atom_selection_loss = torch.zeros((), dtype=torch.float32)
    atom_usage_bonus = torch.zeros((), dtype=torch.float32)
    expressive_gain = torch.zeros((), dtype=torch.float32)
    expressive_gain_rate = 0.0
    ...
    for slot_idx in range(model.num_slots):
        ...
        slot_token_logits = model.slot_token_logits(hidden, slot_idx)
        slot_phrase_probs = F.softmax(slot_token_logits / float(args.phrase_temp), dim=-1)
        phrase_entropy = phrase_entropy + (
            -(slot_phrase_probs.clamp_min(1e-9) * slot_phrase_probs.clamp_min(1e-9).log()).sum(dim=-1).mean()
        )
        mixture = family_probs.unsqueeze(2) * slot_atom_probs
        token_dist = torch.einsum("nfv,nfvpb->npb", mixture, slot_phrase_probs)
        ...
        slot_token_logits_detached = slot_token_logits.detach()
        hard_atom = atom_logits[slot_idx].detach()[row_idx, hard_family].argmax(dim=1)
        hard_phrase = slot_token_logits_detached[row_idx, hard_family, hard_atom].argmax(dim=-1).to(torch.int64)
        ...
        supervised_atom_logits = atom_logits[slot_idx][row_idx, init_family_ids]
        expressive_variant_tokens = slot_token_logits_detached[row_idx, init_family_ids].argmax(dim=-1).to(torch.int64)
        expressive_variant_corrections = (expressive_variant_tokens != target.unsqueeze(1)).sum(dim=2)
        best_expressive_corrections, best_atom_target = expressive_variant_corrections.min(dim=1)
        atom_selection_loss = atom_selection_loss + F.cross_entropy(supervised_atom_logits, best_atom_target)
        base_variant_tokens = base_phrase_tokens[:, slot_idx][init_family_ids].to(torch.int64)
        base_variant_corrections = (base_variant_tokens != target.unsqueeze(1)).sum(dim=2)
        best_base_corrections = base_variant_corrections.min(dim=1).values.to(torch.float32)
        slot_expressive_gain = (best_base_corrections - best_expressive_corrections.to(torch.float32)).clamp_min(0.0)
        expressive_gain = expressive_gain + (slot_expressive_gain / float(model.phrase_len)).mean()
        expressive_gain_rate += float((slot_expressive_gain > 0.0).to(torch.float32).mean().item())
        atom_usage_bonus = atom_usage_bonus - (slot_decision * (slot_expressive_gain / float(model.phrase_len))).mean()
    ...
    phrase_entropy = phrase_entropy / float(model.num_slots)
    decision_loss = decision_loss / float(model.num_slots)
    atom_selection_loss = atom_selection_loss / float(model.num_slots)
    atom_usage_bonus = atom_usage_bonus / float(model.num_slots)
    expressive_gain = expressive_gain / float(model.num_slots)
    expressive_gain_rate = expressive_gain_rate / float(model.num_slots)
    ...
    loss = loss + float(args.atom_selection_weight) * atom_selection_loss
    loss = loss + float(args.atom_usage_bonus_weight) * atom_usage_bonus
    ...
    with torch.no_grad():
        ...
        for slot_idx in range(model.num_slots):
            ...
            hard_atom = selected_atom_logits.argmax(dim=1)
            slot_token_logits = model.slot_token_logits(hidden, slot_idx)
            hard_program[:, slot_idx * model.phrase_len:(slot_idx + 1) * model.phrase_len] = (
                slot_token_logits[row_idx, hard_family, hard_atom].argmax(dim=-1).to(torch.uint8)
            )
        hard_token_match = float((hard_program == sequences).to(torch.float32).mean().item())
        hard_cost = _hard_lha_program_cost(hard_program, sequences, hard_use_atom, model.phrase_len)
    ...
    stats = {
        ...
        "atom_selection_loss": float(atom_selection_loss.item()),
        "atom_usage_bonus": float(atom_usage_bonus.item()),
        "expressive_gain": float(expressive_gain.item()),
        "expressive_gain_rate": float(expressive_gain_rate),
...
def _train_lha_isa(
    ...
    if len(enc.groups) != 1:
        raise ValueError("WAL-LHA Step 12c currently expects exactly one group per target layer")
    group = enc.groups[0]
    ...
    model = HighExpressivityAtomISAEncoder(
        ...
        atom_hidden_dim=int(args.atom_hidden_dim),
    ...
    for step in range(1, int(args.train_steps) + 1):
        ...
        if step == 1 or step % log_every == 0 or step == int(args.train_steps):
            ...
            print(
                f"[wal-lha/train] {name}: step={step}/{args.train_steps} loss={stats['loss']:.4f} "
    ...
    sample_family_orig, _, sample_use_atom, sample_approx, sample_margin, sample_decision_prob = _predict_lha_hard_from_features(
        model,
        norm_features,
            ...
            family_sample_summaries.append(
                {
                    ...
                    "sample_lha_only_token_match": float((sample_approx[mask] == sample_sequences[mask]).to(torch.float32).mean().item()),
                    "sample_lha_only_output_rel_mse": float(sample_output_rel[mask].mean().item()),
                }
            )
        else:
            ...
            family_sample_summaries.append(
                {
                    ...
                    "sample_lha_only_token_match": 0.0,
                    "sample_lha_only_output_rel_mse": 0.0,
                }
            )
    ...
    return {
        ...
        "base_phrase_tokens": model.base_phrase_tokens().cpu(),
...
def _build_lha_layer(
    ...
    group = trained["group"]
    model = trained["model"]
    base_phrase_tokens = trained["base_phrase_tokens"]
    ...
    for start in range(0, rows, int(args.assign_chunk_size)):
        ...
        hard_family_orig, hard_atom_ids, hard_use_atom, approx_chunk, logit_margin, decision_prob = _predict_lha_hard_from_features(
            model,
            norm_features,
            int(args.assign_chunk_size),
        )
    ...
    lha_only = GroupedBlockRVQEncoding(groups=(approx_group,), row_slices=enc.row_slices, original_shape=enc.original_shape)
    family_summaries = []
            ...
            family_summaries.append(
                {
                    ...
                    "lha_only_token_match": float(1.0 - family_token_match_hamming[family_idx].item() / max(count * raw_len, 1)),
    ...
    exact_stats = {
        ...
        "lha_only_avg_hamming": float(total_lo_hamming / max(rows, 1)),
        "lha_only_token_match": float(1.0 - total_lo_hamming / max(num_tokens, 1)),
        "sample_lha_only_token_match": float(trained["sample_token_match"]),
    ...
    artifact = {
        ...
        "feature_centroids": feature_centroids,
        "sequence_centroids": sequence_centroids,
        "base_phrase_tokens": base_phrase_tokens[trained["family_order"]].clone(),
    ...
    return lha_only, exact_stats, artifact
...
def main() -> None:
    ...
    parser.add_argument("--lha-calibration-windows", type=int, default=2)
    ...
    parser.add_argument("--atom-hidden-dim", type=int, default=32)
    ...
    parser.add_argument("--atom-selection-weight", type=float, default=0.10)
    parser.add_argument("--atom-usage-bonus-weight", type=float, default=0.08)
    ...
    parser.add_argument("--bootstrap-cache", default=str(ROOT / "dwl2_dynamic_route/results/m27_wal_lo_current_l54_gate_up.pt"))
    parser.add_argument("--current-cache", default=str(ROOT / "dwl2_dynamic_route/results/m27_wal_lha_current_l54_gate_up.pt"))
    parser.add_argument("--lha-only-cache", default=str(ROOT / "dwl2_dynamic_route/results/m27_wal_lha_only_l54_gate_up.pt"))
    parser.add_argument("--lha-artifact", default=str(ROOT / "dwl2_dynamic_route/results/m27_wal_lha_isa_l54_gate_up.pt"))
    parser.add_argument("--out", default=str(ROOT / "dwl2_dynamic_route/results/m27_wal_lha_proto_summary.json"))
    ...
    layer_inputs = _collect_layer_inputs(ids, TARGETS, int(args.lha_calibration_windows), int(args.activation_rows))
    activation_banks = {
        name: _build_activation_bank(layer_inputs[name], int(args.block_size))
        for name in TARGETS
    }

    lha_only_encodings: dict[str, GroupedBlockRVQEncoding] = {}
    wal_lha_rows = []
    lha_only_compare = []
    artifacts = {}
    for name in TARGETS:
        lha_only, exact_stats, artifact = _build_lha_layer(name, current_enc[name], activation_banks[name], args)
        lha_only_encodings[name] = lha_only
        wal_lha_rows.append(exact_stats)
        lha_only_compare.append({"name": name, **_compare_encodings(current_enc[name], lha_only)})
        artifacts[name] = artifact
        print(
            f"[wal-lha] {name}: avg_program={exact_stats['avg_program_length']:.3f}/{exact_stats['raw_program_length']} "
            f"low_calls={exact_stats['avg_low_level_calls']:.3f} entropy={exact_stats['family_entropy']:.3f} "
            f"lha_only_match={exact_stats['lha_only_token_match']:.3f} sample_out_rel={exact_stats['sample_lha_only_output_rel_mse']:.3f}",
            flush=True,
        )

    lha_artifact = Path(args.lha_artifact)
    lha_artifact.parent.mkdir(parents=True, exist_ok=True)
    torch.save(
        {
            "version": 1,
    ...
    lha_only_cache = Path(args.lha_only_cache)
    save_grouped_encoding_map(lha_only_cache, lha_only_encodings)
    ...
    print(f"\n[eval] lha_only via {args.matmul_strategy}", flush=True)
    lha_only_eval = _run_preencoded_eval(ids, lha_only_cache, args.matmul_strategy, args.num_windows)
    ...
    result = {
        ...
        "lha_only_cache": str(lha_only_cache),
        "lha_artifact": str(lha_artifact),
        "wal_lha": wal_lha_rows,
        "lha_only_compare": lha_only_compare,
        "current_eval": current_eval,
        "exact_eval": exact_eval,
        "lha_only_eval": lha_only_eval,
        "delta_exact": {"ppl_delta": 0.0, "tok_s_delta": 0.0, "peak_mb_delta": 0.0},
        "delta_lha_only": {
            "ppl_delta": float(lha_only_eval["metrics"]["perplexity"] - current_eval["metrics"]["perplexity"]),
            "tok_s_delta": float(lha_only_eval["metrics"]["tok_s"] - current_eval["metrics"]["tok_s"]),
            "peak_mb_delta": float(lha_only_eval["eval_peak_mb"] - current_eval["eval_peak_mb"]),
        },
    }
    ...
    print(
        f"  wal_lha_exact: ppl={exact_eval['metrics']['perplexity']:.4f} tok/s={exact_eval['metrics']['tok_s']:.2f} peak_mb={exact_eval['eval_peak_mb']:.1f}",
        flush=True,
    )
    print(
        f"  lha_only:     ppl={lha_only_eval['metrics']['perplexity']:.4f} tok/s={lha_only_eval['metrics']['tok_s']:.2f} peak_mb={lha_only_eval['eval_peak_mb']:.1f}",
        flush=True,
    )
    for row, compare in zip(wal_lha_rows, lha_only_compare):
        print(
            ...
            f"lha_only_rel_mse={compare['recon_rel_mse']:.6f}",
            flush=True,
        )
    print(
        f"  delta(lha_only-current): ppl={result['delta_lha_only']['ppl_delta']:+.6f} "
        f"tok/s={result['delta_lha_only']['tok_s_delta']:+.2f} peak_mb={result['delta_lha_only']['peak_mb_delta']:+.1f}",
        flush=True,
    )

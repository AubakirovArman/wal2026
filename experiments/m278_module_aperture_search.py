...
editing effectiveness. We find the optimal subset.
"""
...
def train_lora_fp32(model, tokenizer, facts, seed, target_modules):
    random.seed(seed)
...
print("=" * 60)
print("M278 — Module Aperture Search in Layer 16")
print("=" * 60)
...
for modules in MODULE_SETS:
    ...
    model = train_lora_fp32(model, tokenizer, FACTS, SEED, modules)
    ...
    print(f"  Survival: {survival}/{len(FACTS)}")
    results.append({"modules": modules, "survival": survival, "total": len(FACTS)})
...
print("SUMMARY")
print("=" * 60)
print(f"  {'Modules':<40} {'Survival':<10}")
print(f"  {'-'*40} {'-'*10}")
for r in results:
    mods = '+'.join(r['modules'])
    print(f"  {mods:<40} {r['survival']}/{r['total']}")

best = max(results, key=lambda x: x['survival'])
print(f"\n🎯 Best modules: {'+'.join(best['modules'])} (survival {best['survival']}/{best['total']})")
print("=" * 60)

with open("experiments/m278_results.json", "w") as f:
    json.dump(results, f, indent=2)
print("✅ Saved to experiments/m278_results.json")

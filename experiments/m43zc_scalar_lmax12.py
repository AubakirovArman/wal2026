...
"""
#!/usr/bin/env python3
"""M43zc: Scalar-only with l_max=12, skipping spiky."""
import torch
...
COARSE_LADDER = [0.5 ** i for i in range(16)]
...
print("Applying scalar encoder (l_max=12, K=128)...")
stats = {"scalar": 0, "skipped": 0}
for name, param in list(model.named_parameters()):
    ...
    total = stats["scalar"] + stats["skipped"]
    if total % 50 == 0:
        print(f"  Encoded {stats['scalar']} scalar, skipped {stats['skipped']}")
...
print(f"\nScalar lmax=12 PPL (first {end_loc} tokens): {ppl.item():.2f}")

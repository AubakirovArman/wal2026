...
gradient overflow during LoRA training. Training adapters in float32
will fix this.

Test:
1. Encode model in fp16 (as before)
2. Train LoRA adapters in fp32
3. Merge adapters back to fp16
4. Test survival and PPL
"""
...
def train_lora_fp32(model, tokenizer, facts):
    for name, p in model.named_parameters():
    ...
    for layer_idx in TARGET_LAYERS:
        ...
        for mod_name in TARGET_MODULES:
            ...
            def make_forward(orig, adapter):
                def forward(x):
                    # Cast input to FP32 for adapter, cast output back to FP16
    ...
    # Merge adapters back to base weights in FP16
    for layer_idx in TARGET_LAYERS:
        layer = model.model.layers[layer_idx]
        for mod_name in TARGET_MODULES:
            mod = getattr(layer.self_attn if mod_name in ['q_proj','k_proj','v_proj','o_proj'] else layer.mlp, mod_name)
            if hasattr(mod, '_adapter'):
                # Cast adapter to FP16 and merge
                delta = mod._adapter.weight.data.to(mod.weight.dtype)
                mod.weight.data += delta
                # Restore original forward
                original_forward = mod.forward.__closure__[0].cell_contents if hasattr(mod.forward, '__closure__') else None
                # Simple forward: just linear with merged weight
...
def run():
    print("=" * 60)
    print("M241 — Float32 Training Fix")
    print("=" * 60)
    ...
    print("\nTraining LoRA in FP32...")
    model = train_lora_fp32(model, tokenizer, FACTS)
    print("Training complete.")
    
    # Test
    survive = check_facts(model, tokenizer, FACTS)
    ppl = get_ppl(model, tokenizer)
    print(f"\nSurvival: {survive}/{len(FACTS)}")
    print(f"PPL: {math.exp(ppl):.4f} (Δ={math.exp(ppl)-math.exp(baseline_ppl):+.4f})")
    
    # Check for nan
    has_nan = any(p.isnan().any() for p in model.parameters())
    print(f"Model has nan: {has_nan}")
    
    output = {
        "baseline_ppl": math.exp(baseline_ppl),
        "encoded_ppl": math.exp(encoded_ppl),
        "edit_ppl": math.exp(ppl) if not math.isnan(ppl) else None,
        "survival": survive,
        "total_facts": len(FACTS),
        "has_nan": has_nan,
    }
    with open("experiments/m241_results.json", "w") as f:
        json.dump(output, f, indent=2)
    print("\n✅ Saved to experiments/m241_results.json")
    
    if survive > 0 and not has_nan and not math.isnan(ppl):
        print("\n✅ FLOAT32 FIX WORKS — training pipeline restored")
    else:
        print("\n❌ FLOAT32 FIX INSUFFICIENT — deeper issue exists")

...
Uses 5 edits instead of 10 for speed.
"""
...
def main():
    print("=" * 60, flush=True)
    print("M233 v2 — WAL Garbage Collection", flush=True)
    print("=" * 60, flush=True)
    ...
    # Baseline: all 5 edits
    print("\n[1/3] Baseline: all 5 edits", flush=True)
    ...
    print(f"  All 5 edits: {base_surv_all}/25 survival", flush=True)
    del model
    gc.collect()
    torch.cuda.empty_cache()
    
    # GC: Remove first 2 edits (oldest)
    print("\n[2/3] GC: Remove edits 1-2 (oldest)", flush=True)
    model, _ = build_model_with_edits(tokenizer, device, list(range(2, N_EDITS)))
    gc_surv_kept = sum(eval_survival(model, tokenizer, device, batch) for batch in batches[2:])
    gc_surv_removed = sum(eval_survival(model, tokenizer, device, batch) for batch in batches[:2])
    print(f"  Kept edits 3-5: {gc_surv_kept}/15 survival", flush=True)
    print(f"  Removed edits 1-2: {gc_surv_removed}/10 survival", flush=True)
    del model
    gc.collect()
    torch.cuda.empty_cache()
    
    # GC: Keep only last 2 edits
    print("\n[3/3] GC: Keep only last 2 edits", flush=True)
    model, _ = build_model_with_edits(tokenizer, device, list(range(3, N_EDITS)))
    gc2_surv = sum(eval_survival(model, tokenizer, device, batch) for batch in batches)
    print(f"  Last 2 edits: {gc2_surv}/25 survival", flush=True)
    del model
    gc.collect()
    torch.cuda.empty_cache()
    ...
    print("-" * 55, flush=True)
    print(f"{'All 5 edits (baseline)':<40} {base_surv_all:>9}/25", flush=True)
    print(f"{'Remove edits 1-2 (oldest)':<40} {gc_surv_kept:>9}/15", flush=True)
    print(f"{'Last 2 edits only':<40} {gc2_surv:>9}/25", flush=True)
    
    results = {
        "baseline_all_5": base_surv_all,
        "gc_remove_oldest_2": {"kept": gc_surv_kept, "removed": gc_surv_removed},
        "gc_last_2": gc2_surv,
    }

...
from dwl2_dynamic_route.src.tile_palette_runtime import benchmark_tile_palette_runtime, estimate_global_bpw, estimate_local_bpw
...
def _summary(entries: list[dict[str, object]]) -> dict[str, float | int]:
    ...
    dsu = [float(item["decode_speedup"]) for item in entries]
    lsu = [float(item["linear_speedup"]) for item in entries]
    local_bpw = [float(item["local_bpw_estimate"]) for item in entries]
    return {
        "tiles": len(entries),
        ...
        "mean_decode_speedup": float(statistics.mean(dsu)),
        "mean_linear_speedup": float(statistics.mean(lsu)),
        "mean_local_bpw_estimate": float(statistics.mean(local_bpw)),
    }
...
def main() -> None:
    ...
    parser.add_argument("--sample-tiles", type=int, default=6)
    parser.add_argument("--use-top-tiles", type=int, default=1)
    parser.add_argument("--palette-sizes", type=int, nargs="+", default=[16, 32])
    ...
    parser.add_argument("--out", default=str(ROOT / "dwl2_dynamic_route/results/m6c_route_distill_layer_suite.json"))
    ...
    for tensor_idx, tensor_name in enumerate(args.tensor_names):
        ...
        sampled_tiles = sample_tile_stats(ids, args.tile_rows, args.tile_cols, args.sample_tiles, args.seed + tensor_idx)
        ...
        global_bpw = estimate_global_bpw(codebook.size)
        for tile_idx, tile in enumerate(selected_tiles):
            ...
            tile_ids = ids[tile.row0:row1, tile.col0:col1]
            for palette_size in args.palette_sizes:
                distilled = distill_tile_palette(
                    ...
                    steps=args.steps,
                    seed=args.seed + tensor_idx * 101 + tile_idx * 17 + palette_size,
                )
                local_repr = torch.tensor(distilled["projected_palette"], device=teacher_tile.device, dtype=torch.float32)
                assign = (teacher_tile.unsqueeze(-1) - local_repr.view(1, 1, -1)).abs().argmin(dim=-1)
                hard_tile = local_repr[assign]
                bench = benchmark_tile_palette_runtime(tile_ids, codebook_sum, hard_tile, bench_tokens=args.bench_tokens)
                sweep[str(palette_size)].append(
                    {
                        ...
                        "global_bpw_estimate": global_bpw,
                        "local_bpw_estimate": estimate_local_bpw(bench["local_unique"], hard_tile.numel()),
                        **distilled,
                        **bench,
                    }
                )
        summary = {key: _summary(entries) for key, entries in sweep.items()}
        suite[tensor_name] = {

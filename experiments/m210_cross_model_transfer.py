...
Test if WAL works on a different model architecture/size.
Compare: Llama-3.1-8B vs Llama-3.2-1B
"""
...
K = 256
ITERS = 3
TARGET_LAYERS = [14, 15, 16]  # Adjust for model size
...
# ── Main ─────────────────────────────────────────────────
def test_model(model_name, model_id, target_layers, device):
    print(f"\n{'='*60}", flush=True)
...
def main():
    print("=" * 60, flush=True)
    print("M210 — Cross-Model Transfer", flush=True)
    print("=" * 60, flush=True)
    
    device = DEVICE
    results = []
    
    # Test Llama-3.1-8B (baseline)
    # results.append(test_model("llama-8b", MODELS["llama-8b"], [14,15,16], device))
    
    # Test Llama-3.2-1B
    # Adjust target layers for 1B model (fewer layers)
    results.append(test_model("llama-1b", MODELS["llama-1b"], [8,9,10], device))
    
    # Summary
    print(f"\n{'='*60}", flush=True)
    print("SUMMARY", flush=True)
    print(f"{'='*60}", flush=True)
    print(f"{'Model':<15} {'Base PPL':>10} {'Enc Δ':>10} {'LoRA Δ':>10} {'Re-enc Δ':>10} {'Survival':>10}", flush=True)
    print("-" * 70, flush=True)
    for r in results:
        print(f"{r['model']:<15} {r['baseline_ppl']:>10.4f} {r['encoded_ppl']-r['baseline_ppl']:>+10.4f} "
              f"{r['lora_ppl']-r['baseline_ppl']:>+10.4f} {r['reencode_ppl']-r['baseline_ppl']:>+10.4f} "
              f"{r['reencode_survival']:>10.0f}/10", flush=True)
    
    with open("experiments/m210_results.json", "w") as f:
        json.dump(results, f, indent=2)
    print("\n✅ Saved to experiments/m210_results.json", flush=True)

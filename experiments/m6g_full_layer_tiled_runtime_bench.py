from __future__ import annotations

import argparse
import json
import sys
from pathlib import Path

import torch
import torch.nn.functional as F

ROOT = Path(__file__).resolve().parents[2]
MODEL_DIR = ROOT / "bk/.hf_cache/hub/models--google--gemma-4-31B-it/snapshots/439edf5652646a0d1bd8b46bfdc1d3645761a445"
sys.path.insert(0, str(ROOT))

from dwl2_dynamic_route.src.calibrate import calibrate_ladder
from dwl2_dynamic_route.src.codebook import build_codebook
from dwl2_dynamic_route.src.full_layer_tiled_runtime import build_exact_local_tile_plan, full_layer_global_tiled_matmul, full_layer_local_tiled_matmul, mean_local_unique
from dwl2_dynamic_route.src.route_encoder import encode_routes
from dwl2_dynamic_route.src.triton_id_matmul import id_route_linear_matmul


def _load_weight(tensor_name: str, device: str) -> torch.Tensor:
    from safetensors import safe_open

    with open(MODEL_DIR / "model.safetensors.index.json") as handle:
        shard_map = json.load(handle)["weight_map"]
    shard_path = MODEL_DIR / shard_map[tensor_name]
    with safe_open(str(shard_path), framework="pt", device=device) as handle:
        return handle.get_tensor(tensor_name)


def _bench_cuda(fn, reps: int = 5, warmup: int = 2) -> float:
    for _ in range(warmup):
        fn()
    torch.cuda.synchronize()
    start = torch.cuda.Event(enable_timing=True)
    end = torch.cuda.Event(enable_timing=True)
    start.record()
    for _ in range(reps):
        fn()
    end.record()
    torch.cuda.synchronize()
    return start.elapsed_time(end) / reps


def _targets(args: argparse.Namespace) -> list[str]:
    if args.tensor_names:
        return args.tensor_names
    with open(args.policy_json) as handle:
        policy = json.load(handle)
    return [item["tensor_name"] for item in policy["approved"]]


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--policy-json", default=str(ROOT / "dwl2_dynamic_route/results/m6f_selective_runtime_policy.json"))
    parser.add_argument("--tensor-names", nargs="+")
    parser.add_argument("--tile-rows", type=int, default=128)
    parser.add_argument("--tile-cols", type=int, default=128)
    parser.add_argument("--bench-tokens", type=int, default=512)
    parser.add_argument("--sample-limit", type=int, default=2_000_000)
    parser.add_argument("--out", default=str(ROOT / "dwl2_dynamic_route/results/m6g_full_layer_tiled_runtime_bench.json"))
    args = parser.parse_args()

    targets = _targets(args)
    if not targets:
        raise ValueError("no tensors selected for m6g bench")
    device = "cuda" if torch.cuda.is_available() else "cpu"
    results: list[dict[str, object]] = []
    for tensor_name in targets:
        weight = _load_weight(tensor_name, device=device)
        row_scale = weight.abs().amax(dim=-1, keepdim=True).clamp_min(1e-8)
        w_norm = weight / row_scale
        sample = w_norm.flatten()
        if sample.numel() > args.sample_limit:
            idx = torch.randint(0, sample.numel(), (args.sample_limit,), device=sample.device)
            sample = sample[idx]
        ladder = calibrate_ladder(sample, l_max=12, refine_iters=20, pin_top=True, top_value=1.0, seed="geometric")
        enc = encode_routes(w_norm, ladder, stop_threshold=0.0, l_max=12)
        codebook, ids = build_codebook(enc.digits, enc.stop_depth, l_max=12)
        codebook_sum = (codebook.digits.to(torch.float32) * ladder.to(torch.float32)).sum(dim=-1)
        routed_norm = codebook_sum[ids.long()]
        dense_w = (routed_norm * row_scale).to(torch.float16).contiguous()
        local_plan = build_exact_local_tile_plan(routed_norm, args.tile_rows, args.tile_cols)
        x = torch.randn(args.bench_tokens, weight.shape[1], device=device, dtype=torch.float16)

        dense_ref = F.linear(x, dense_w)
        global_ref = id_route_linear_matmul(x, ids, codebook_sum, row_scale)
        local_ref = full_layer_local_tiled_matmul(x, local_plan, row_scale)

        ms_dense = _bench_cuda(lambda: F.linear(x, dense_w))
        ms_global_full = _bench_cuda(lambda: id_route_linear_matmul(x, ids, codebook_sum, row_scale))
        ms_global_tiled = _bench_cuda(lambda: full_layer_global_tiled_matmul(x, ids, codebook_sum, row_scale, args.tile_rows, args.tile_cols))
        ms_local_tiled = _bench_cuda(lambda: full_layer_local_tiled_matmul(x, local_plan, row_scale))
        result = {
            "tensor_name": tensor_name,
            "shape": [int(weight.shape[0]), int(weight.shape[1])],
            "global_unique_routes": int(codebook.size),
            "mean_exact_local_unique": mean_local_unique(local_plan),
            "ms_dense": ms_dense,
            "ms_global_full": ms_global_full,
            "ms_global_tiled": ms_global_tiled,
            "ms_local_tiled": ms_local_tiled,
            "local_vs_global_full": ms_global_full / max(ms_local_tiled, 1e-12),
            "local_vs_global_tiled": ms_global_tiled / max(ms_local_tiled, 1e-12),
            "local_vs_dense": ms_dense / max(ms_local_tiled, 1e-12),
            "global_rel_mse": float(torch.mean((global_ref - dense_ref).float().square()).item()),
            "local_rel_mse": float(torch.mean((local_ref - dense_ref).float().square()).item()),
        }
        results.append(result)
        print(
            f"[m6g] {tensor_name} local/global_full={result['local_vs_global_full']:.2f}x "
            f"local/global_tiled={result['local_vs_global_tiled']:.2f}x local/dense={result['local_vs_dense']:.2f}x"
        )

    out_path = Path(args.out)
    out_path.parent.mkdir(parents=True, exist_ok=True)
    out_path.write_text(json.dumps({"results": results}, indent=2))
    print(f"[m6g] wrote {out_path}")


if __name__ == "__main__":
    main()
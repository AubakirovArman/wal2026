...
M195 used uniform quantization. This version uses sklearn MiniBatchKMeans
for better accuracy while avoiding GPU OOM via CPU computation.
"""
...
def kmeans_quantize_cpu(h, K):
    """MiniBatchKMeans on CPU for scalar quantization."""
    from sklearn.cluster import MiniBatchKMeans
    
    h_np = h.reshape(-1).cpu().numpy().reshape(-1, 1)
    ...
    quantized = torch.tensor(centers[labels], device=h.device, dtype=h.dtype).reshape(h.shape)
    atoms = torch.tensor(centers, device=h.device, dtype=h.dtype)
    indices = torch.tensor(labels, device=h.device, dtype=torch.int64)
    
    return quantized, atoms, indices


def hadamard_wal_encode(w, K):
    """Hadamard-WAL encode: transform → k-means quantize (CPU)."""
    h, orig_info = hadamard_transform_2d(w.float())
    quantized, atoms, indices = kmeans_quantize_cpu(h, K)
...
def main():
    ...
    # Uniform K=256
    print("\n--- Uniform K=256 Hadamard-WAL + k-means ---")
    total_bits = 0
    ...
    print("\n--- Adaptive K Hadamard-WAL + k-means ---")
    model = AutoModelForCausalLM.from_pretrained(
        "meta-llama/Llama-3.1-8B",
        torch_dtype=torch.bfloat16,
        device_map=device,
    ...
    with open('/mnt/hf_model_weights/arman/3bit/wal/experiments/m195b_hadamard_adaptive_kmeans.json', 'w') as f:

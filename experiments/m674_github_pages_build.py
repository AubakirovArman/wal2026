...
python -m wal validate-results experiments --fail-on-invalid</code></pre>
  ...
  <div class="grid">
    ...
    <div class="card"><strong>AIGI loop</strong><br>M679-M694: {aigi.get("fact_learning_passed", 0)} learned facts, {aigi.get("bad_memory_rejected", 0)} bad-memory rejections, {aigi.get("feedback_episodes_passed", 0)} feedback episodes, {aigi.get("real_hf_backend_checks_passed", 0)} real-HF checks, {aigi.get("soft_prompt_adapter_checks_passed", 0)} adapter checks.</div>
  </div>
  ...
  <ul>
    ...
    <li>AIGI M679-M694 are SDK memory-loop, real-inference, and real soft-prompt adapter gates, not autonomous AGI or LoRA/MEMIT base-weight editing.</li>

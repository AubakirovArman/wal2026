...
python -m wal validate-results experiments --fail-on-invalid</code></pre>
  ...
  <div class="grid">
    ...
    <div class="card"><strong>AIGI loop</strong><br>M679-M687: {aigi.get("fact_learning_passed", 0)} learned facts, {aigi.get("bad_memory_rejected", 0)} bad-memory rejections, {aigi.get("feedback_episodes_passed", 0)} feedback episodes, {aigi.get("contract_rollback_checks_passed", 0)} contract rollback checks.</div>
  </div>
  ...
  <ul>
    ...
    <li>AIGI M679-M687 are SDK memory-loop proofs, not autonomous AGI or real semantic weight editing.</li>

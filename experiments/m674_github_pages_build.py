...
python -m wal validate-results experiments --fail-on-invalid</code></pre>
  ...
  <div class="grid">
    ...
    <div class="card"><strong>AIGI loop</strong><br>M679-M695: {aigi.get("fact_learning_passed", 0)} learned facts, {aigi.get("bad_memory_rejected", 0)} bad-memory rejections, {aigi.get("feedback_episodes_passed", 0)} feedback episodes, {aigi.get("real_hf_backend_checks_passed", 0)} real-HF checks, {aigi.get("soft_prompt_adapter_checks_passed", 0)} soft-prompt checks, {aigi.get("logit_lora_adapter_checks_passed", 0)} logit-LoRA checks.</div>
  </div>
  ...
  <ul>
    ...
    <li>AIGI M679-M695 are SDK memory-loop, real-inference, soft-prompt, and logit-LoRA adapter gates, not autonomous AGI or attention/MLP LoRA/MEMIT base-weight editing.</li>

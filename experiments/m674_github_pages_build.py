...
python -m wal validate-results experiments --fail-on-invalid</code></pre>
  ...
  <div class="grid">
    ...
    <div class="card"><strong>AIGI loop</strong><br>M679-M693: {aigi.get("fact_learning_passed", 0)} learned facts, {aigi.get("bad_memory_rejected", 0)} bad-memory rejections, {aigi.get("feedback_episodes_passed", 0)} feedback episodes, {aigi.get("budget_checks_passed", 0)} budget checks, {aigi.get("real_hf_backend_checks_passed", 0)} real-HF checks.</div>
  </div>
  ...
  <ul>
    ...
    <li>AIGI M679-M693 are SDK memory-loop and real-inference gates, not autonomous AGI or real semantic weight editing.</li>

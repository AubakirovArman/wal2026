...
Repeating with FP32 adapters + layer 16 + seed=42 will produce
first real PASS/FAIL CI.
"""
...
def ci_verdict(results, baseline_ppl):
    ...
    checks.append(("ppl_gate", ppl_ok, f"{ppl_val:.2f}"))
...
def run():
    print("=" * 60)
    print("M252 — WAL CI Pipeline with FP32 Fix")
    print("=" * 60)
    print("Stack: Layer 16 + FP32 adapters + seed=42")
    
    model, tokenizer = load_model()
    baseline_ppl = get_ppl(model, tokenizer)
    print(f"\n[Stage 1] Baseline PPL: {math.exp(baseline_ppl):.4f}")
    del model
    gc.collect()
    ...
    print("\n[Stage 3] Edit (layer 16, fp32)")
    model = train_lora_fp32(model, tokenizer, TEST_FACTS)
    ...
    nan_ok = not has_nan
    print(f"  {'no_nan':<15} {'✅ PASS' if nan_ok else '❌ FAIL'} ({nan_ok})")
    
    overall = "✅ PASS" if (passed and nan_ok) else "❌ FAIL"
    print(f"\n  OVERALL: {overall}")
    ...
    output = {
        ...
        "no_nan": nan_ok,
        "overall_pass": passed and nan_ok,
    }
    with open("experiments/m252_results.json", "w") as f:
        json.dump(output, f, indent=2)
    print("\n✅ Saved to experiments/m252_results.json")

...
"""
#!/usr/bin/env python3
"""M43h: Encode all params and check for NaN/Inf."""
import torch
...
for name, param in list(model.named_parameters()):
    ...
    w_hat = w_hat.to(param.dtype)
    param.data.copy_(w_hat)

    # Check for issues
    has_nan = param.data.isnan().any().item()
    has_inf = param.data.isinf().any().item()
    if has_nan or has_inf:
        print(f"PROBLEM: {name} NaN={has_nan} Inf={has_inf}")

    # Check if all zeros
    all_zero = (param.data == 0).all().item()
    if all_zero:
        print(f"PROBLEM: {name} all zeros!")

print("All params checked.")

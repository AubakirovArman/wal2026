...
"""
"""
M205b — Stratified Risk Dataset

Add extreme configs for balanced survival spectrum:
- steps=200 (strong edits, survival ~7-15)
- Mix of rank, layers, modules, lr
"""
...
def train_lora(model, tokenizer, steps, rank, target_layers, target_modules, lr, device):
...
def main():
    print("=" * 60, flush=True)
    print("M205b — Stratified Risk Dataset", flush=True)
    print("=" * 60, flush=True)
    
    # Load M205 data
    existing = []
    with open("experiments/m205_data.csv", "r") as f:
        reader = csv.DictReader(f)
        for row in reader:
            existing.append(row)
    print(f"Existing data points: {len(existing)}", flush=True)
    
    # Stratified probes: steps=200 for strong edits
    PROBE_CONFIGS = [
        {"rank": 4, "wave_lambda": 0.0, "n_modules": 4, "n_layers": 3, "steps": 200, "lr": 5e-5, "layers": [14,15,16], "modules": ["o_proj","q_proj","v_proj","gate_proj"]},
        {"rank": 8, "wave_lambda": 0.0, "n_modules": 4, "n_layers": 3, "steps": 200, "lr": 5e-5, "layers": [14,15,16], "modules": ["o_proj","q_proj","v_proj","gate_proj"]},
        {"rank": 4, "wave_lambda": 0.0, "n_modules": 6, "n_layers": 3, "steps": 200, "lr": 5e-5, "layers": [14,15,16], "modules": ["o_proj","q_proj","v_proj","gate_proj","up_proj","down_proj"]},
        {"rank": 4, "wave_lambda": 0.0, "n_modules": 4, "n_layers": 5, "steps": 200, "lr": 5e-5, "layers": [12,13,14,15,16], "modules": ["o_proj","q_proj","v_proj","gate_proj"]},
        {"rank": 4, "wave_lambda": 0.0, "n_modules": 4, "n_layers": 3, "steps": 200, "lr": 1e-4, "layers": [14,15,16], "modules": ["o_proj","q_proj","v_proj","gate_proj"]},
        {"rank": 4, "wave_lambda": 0.0, "n_modules": 4, "n_layers": 3, "steps": 400, "lr": 5e-5, "layers": [14,15,16], "modules": ["o_proj","q_proj","v_proj","gate_proj"]},
        {"rank": 8, "wave_lambda": 0.0, "n_modules": 4, "n_layers": 3, "steps": 400, "lr": 5e-5, "layers": [14,15,16], "modules": ["o_proj","q_proj","v_proj","gate_proj"]},
        {"rank": 4, "wave_lambda": 0.0, "n_modules": 6, "n_layers": 5, "steps": 400, "lr": 5e-5, "layers": [12,13,14,15,16], "modules": ["o_proj","q_proj","v_proj","gate_proj","up_proj","down_proj"]},
    ]
    ...
    for ci, cfg in enumerate(PROBE_CONFIGS):
        ...
        print(f"  rank={cfg['rank']}, steps={cfg['steps']}, modules={cfg['n_modules']}, layers={cfg['n_layers']}, lr={cfg['lr']}", flush=True)
        ...
        print(f"  PPL={ppl:.4f}, Survival={survival}/50, mean_sn={mean_sn:.4f}, max_sn={max_sn:.4f}, time={train_time:.1f}s", flush=True)
        
        new_data.append({
            "rank": cfg['rank'], "wave_lambda": cfg['wave_lambda'],
            "n_modules": cfg['n_modules'], "n_layers": cfg['n_layers'],
            "steps": cfg['steps'], "lr": cfg['lr'],
            "mean_spectral_norm": mean_sn, "max_spectral_norm": max_sn,
            "mean_top10_energy": mean_energy, "max_top10_energy": max_energy,
            "final_loss": ppl, "survival": survival,
        })
    ...
    with open("experiments/m205b_data.csv", "w", newline="") as f:
    ...
    print("\n✅ Saved to experiments/m205b_data.csv", flush=True)
    ...
    try:
        ...
        for row in all_data:
            X.append([
                float(row['rank']), float(row['wave_lambda']), float(row['n_modules']),
                float(row['n_layers']), float(row['steps']), float(row['lr']),
                float(row['mean_spectral_norm']), float(row['max_spectral_norm']),
                float(row['mean_top10_energy']), float(row['max_top10_energy']),
        ...
        feature_names = ['rank', 'wave_lambda', 'n_modules', 'n_layers', 'steps', 'lr',
        ...
        import pickle
        with open("experiments/m205b_model.pkl", "wb") as f:
            pickle.dump(rf, f)
        print("\n✅ Model saved to experiments/m205b_model.pkl", flush=True)

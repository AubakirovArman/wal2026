...
Maybe single-module is where wave reg actually works.

Grid: λ ∈ [0.0, 0.01, 0.025, 0.05, 0.1]
Runs: n=10 per λ (single target = lower variance)
"""
...
DEVICE = "cuda"
LAMBDA_GRID = [0.0, 0.01, 0.025, 0.05, 0.1]
N_RUNS = 10
RANK = 4
STEPS = 100
LR = 5e-5
TARGET_LAYERS = [14, 15, 16]
TARGET_MODULES = ["o_proj"]  # SINGLE MODULE only
...
def main():
    print("=" * 60, flush=True)
    print("M196g — Wave-LoRA Single-Module (o_proj only, n=10)", flush=True)
    print("=" * 60, flush=True)
    device = DEVICE
    ...
    print("\n" + "=" * 60, flush=True)
    print("Summary (n=10 runs, o_proj only)", flush=True)
    print("=" * 60, flush=True)
    ...
    with open("experiments/m196g_results.json", "w") as f:
        json.dump(all_results, f, indent=2)
    print("\n✅ Saved to experiments/m196g_results.json", flush=True)

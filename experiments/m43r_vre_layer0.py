#!/usr/bin/env python3
"""M43r: Encode all 5 params of layer 0 with VRE."""
import torch
...
for name, param in list(model.named_parameters()):
    ...
    print(f"Encoded {name}: rel_mse={((w - w_hat)**2).mean() / (w**2).mean():.6f}")
...
print(f"\nVRE layer0-all PPL (first {end_loc} tokens): {ppl.item():.2f}")

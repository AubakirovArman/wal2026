...
def train_lora_fp32(model, tokenizer, facts, seed, target_layers):
    random.seed(seed)
...
print("=" * 60)
print("M277 — Layer Aperture Sweep")
print("=" * 60)
...
for layer_idx in LAYERS_TO_TEST:
    ...
    model = train_lora_fp32(model, tokenizer, FACTS, SEED, [layer_idx])
    ...
    results.append({"layer": layer_idx, "survival": survival, "total": len(FACTS)})
...
print("SUMMARY")
print("=" * 60)
print(f"  {'Layer':<8} {'Survival':<10}")
print(f"  {'-'*8} {'-'*10}")
for r in results:
    print(f"  {r['layer']:<8} {r['survival']}/{r['total']}")

best = max(results, key=lambda x: x['survival'])
print(f"\n🎯 Best layer: {best['layer']} (survival {best['survival']}/{best['total']})")
if best['layer'] == 16:
    print("✅ Layer 16 confirmed as optimal")
print("=" * 60)

with open("experiments/m277_results.json", "w") as f:
    json.dump(results, f, indent=2)
print("✅ Saved to experiments/m277_results.json")

...
def main() -> None:
    ...
    parser.add_argument("--group-rows", type=int, nargs="+", default=[128, 256, 512])
    parser.add_argument("--group-cols", type=int, nargs="+", default=[2048, 8192])
    parser.add_argument("--bench-tokens", type=int, default=512)
    parser.add_argument("--sample-limit", type=int, default=2_000_000)
    parser.add_argument("--out", default=str(ROOT / "dwl2_dynamic_route/results/m6i_grouped_2d_runtime_bench.json"))
    ...
    for tensor_name in targets:
        ...
        for group_rows in args.group_rows:
            group_rows = min(group_rows, weight.shape[0])
            for group_cols in args.group_cols:
                group_cols = min(group_cols, weight.shape[1])
                plan = build_grouped_local_plan(routed_norm, group_rows, group_cols)
                local_ref = full_layer_grouped_local_matmul(x, plan, row_scale)
                global_ref = full_layer_grouped_global_matmul(x, ids, codebook_sum, row_scale, group_rows, group_cols)
                ms_local = _bench_cuda(lambda: full_layer_grouped_local_matmul(x, plan, row_scale))
                ms_global = _bench_cuda(lambda: full_layer_grouped_global_matmul(x, ids, codebook_sum, row_scale, group_rows, group_cols))
                results.append(
                    {
                ...
                print(
                    f"[m6i] {tensor_name} gr={group_rows:>4} gc={group_cols:>4} launches={len(plan):>5} "

...
- 800 milestone experiment scripts in `experiments/m*.py`
- 822 Python scripts total in `experiments/`
- 490 experiment result JSON files in `experiments/`
- 635 book/diary entries in `book/`
- 274 top-level result files in `results/`
- 235 docs files in `docs/`
- 5570+ lines in `docs/dev_diary_ru.md`
- 95 Python source modules in `src/`
- 35 pytest tests for the packaged core, audit helpers, and AIGI SDK

## Repository Map

```text
experiments/        milestone scripts and result JSON for M1-M697+
...
`src/aigi/` starts a separate pre-alpha AIGI SDK layer above WAL. It implements a verified memory loop: propose memory, select tier, verify gates, commit, reject, rollback, and log the result. M679-M697 validate 100 synthetic memory updates, 20 bad-memory rejections, tier routing, rollback, behavioral contracts, experience-to-memory extraction, 25 verified feedback episodes, memory budgets, risk ledger, regression suite, and commit decision reports, a real Qwen2.5-0.5B-Instruct HF inference backend gate, a real gradient-trained soft-prompt adapter gate, a real low-rank logit-LoRA adapter gate, a real MLP module-LoRA adapter gate, and fresh-model module-LoRA artifact reload. This is not an AGI claim and does not attach a production semantic weight-edit backend yet.
...
python experiments/m695_aigi_real_logit_lora_adapter.py
python experiments/m696_aigi_real_module_lora_adapter.py
python experiments/m697_aigi_real_module_lora_reload.py
```

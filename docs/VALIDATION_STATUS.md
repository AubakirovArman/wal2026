...
|------|------|--------|-------|
| Core package | `pytest -q tests` | PASS | 34 maintained tests pass locally. |
| Result schema | `python -m wal validate-results experiments --fail-on-invalid` | PASS | 485/485 result files valid. |
| Inventory | M624 | PASS | 817 experiment scripts, 0 parse failures. |
| Safe sweep | M625 | PASS | 289 safe scripts pass, 528 scripts blocked by policy. |
...
| Legacy manifest | M677 | PASS | 817 scripts classified by runner type and review status. |
...
- AIGI M679-M692 do not prove autonomous AGI or real semantic weight editing; `wal_recipe` is still a recipe artifact plus retrieval overlay.

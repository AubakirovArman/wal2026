...
|------|------|--------|-------|
| Core package | `pytest -q tests` | PASS | 25 maintained tests pass locally. |
| Result schema | `python -m wal validate-results experiments --fail-on-invalid` | PASS | 476/476 result files valid. |
| Inventory | M624 | PASS | 808 experiment scripts, 0 parse failures. |
| Safe sweep | M625 | PASS | 280 safe scripts pass, 528 scripts blocked by policy. |
| Public claims | M630/M671 | PASS | Release-facing docs keep pre-alpha wording. |
| Docs smoke | M631 | PASS | 60/60 fast reviewer commands pass. |
...
| Legacy manifest | M677 | PASS | 808 scripts classified by runner type and review status. |
...
| AIGI verified memory loop | M679-M683 | PASS | M679: 7 positive/4 negative; M680: 100/100 facts; M681: 20/20 rejected bad memories; M682: 9/9 routing checks; M683: 8/8 rollback checks. |
...
- AIGI M679-M683 do not prove autonomous AGI or real semantic weight editing; `wal_recipe` is still a recipe artifact plus retrieval overlay.

...
| Result schema | `python -m wal validate-results experiments --fail-on-invalid` | PASS | 480/480 result files valid. |
| Inventory | M624 | PASS | 812 experiment scripts, 0 parse failures. |
| Safe sweep | M625 | PASS | 284 safe scripts pass, 528 scripts blocked by policy. |
| Public claims | M630/M671 | PASS | Release-facing docs keep pre-alpha wording. |
| Docs smoke | M631 | PASS | 64/64 fast reviewer commands pass. |
...
| Legacy manifest | M677 | PASS | 812 scripts classified by runner type and review status. |

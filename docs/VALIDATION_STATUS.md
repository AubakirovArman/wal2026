...
| Result schema | `python -m wal validate-results experiments --fail-on-invalid` | PASS | 487/487 result files valid. |
| Inventory | M624 | PASS | 819 experiment scripts, 0 parse failures. |
| Safe sweep | M625 | PASS | 289 safe scripts pass, 530 scripts blocked by policy. |
...
| Legacy manifest | M677 | PASS | 819 scripts classified by runner type and review status. |
...
- AIGI M679-M694 do not prove autonomous AGI or base-weight semantic editing; M693 is real HF inference and M694 is real soft-prompt adapter training, while `wal_recipe` is still not a LoRA/MEMIT backend.

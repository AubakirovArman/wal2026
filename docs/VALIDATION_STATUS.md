...
| Result schema | `python -m wal validate-results experiments --fail-on-invalid` | PASS | 486/486 result files valid. |
| Inventory | M624 | PASS | 818 experiment scripts, 0 parse failures. |
| Safe sweep | M625 | PASS | 289 safe scripts pass, 529 scripts blocked by policy. |
...
| Legacy manifest | M677 | PASS | 818 scripts classified by runner type and review status. |
...
- AIGI M679-M693 do not prove autonomous AGI or real semantic weight editing; M693 is real HF inference integration, while `wal_recipe` is still a recipe artifact plus retrieval overlay.

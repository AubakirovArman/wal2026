...
| Матрицы в `layer 24` | Q/K/V/O + 68.45703125% up + 2.9296875% gate + 6.15234375% down | 31.54296875% up + 97.0703125% gate + 93.84765625% down |
| Крупные матрицы, включая tied embedding/head | 11 / 197 | 186 |
| Крупные matrix weights | 72,671,232 / 1,720,451,072 | 1,647,779,840 |

Покрытие крупных matrix weights равно `4.223964%`. Нельзя округлять частично
...
Принятые 72,671,232 weights занимают 138.6094 MiB в BF16. После настоящей
Q2-g128 упаковки их расчётный payload составит 18.4091 MiB, экономия —
120.2003 MiB. Экономия VRAM появится только после packed runtime; fake-quant
обучение её не даёт.
...
В `layer 24` приняты 68.45703125% g128-групп `up_proj`, 2.9296875%
`gate_proj` и 6.15234375% `down_proj`. Остальные 31.54296875% `up_proj`,
97.0703125% `gate_proj` и 93.84765625% `down_proj` пока остаются BF16.
...
|---|---:|---:|---:|
| v3 | 0.997087 | 1.000884 | 0.981072 (PyTorch) |
| v4 | 0.997087 | 0.997200 | 0.980767 (Transformers) |
...
1. **NLL budget.** Для условной цели `+5% NLL` и coverage `c` используем
   диагностическую линию `1 + 0.05*c`. При текущем `c=0.04223964` это
   `1.00211198`.
...
```text
wal2/checkpoints/wal-tat-block24_gate_after_recovery_s0014-candidate_bf16_mlp.pt
```

- размер: `304,659,816` bytes;
- SHA-256: `c79e7b032449c6d1dc275d487dcdbc78ec9d7103ce10014260744bcb6a22a313`;
- содержание: полный ternary block 27, Q/K/V/O block 24, 68.45703125%
  `up_proj`, 2.9296875% `gate_proj` и 6.15234375% `down_proj`;
...
1. продолжить round-robin с `up_proj +0.78125%` атомом;

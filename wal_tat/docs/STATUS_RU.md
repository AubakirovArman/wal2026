...
| Матрицы в `layer 24` | Q/K/V/O + 41.89453125% up + 0.1953125% gate + 1.07421875% down | 58.10546875% up + 99.8046875% gate + 98.92578125% down |
| Крупные матрицы, включая tied embedding/head | 11 / 197 | 186 |
| Крупные matrix weights | 68,345,856 / 1,720,451,072 | 1,652,105,216 |

Покрытие крупных matrix weights равно `3.972554%`. Нельзя округлять частично
...
Принятые 68,345,856 weights занимают 130.3594 MiB в BF16. После настоящей
Q2-g128 упаковки их расчётный payload составит 17.3134 MiB, экономия —
113.0460 MiB. Экономия VRAM появится только после packed runtime; fake-quant
обучение её не даёт.
...
В `layer 24` приняты 41.89453125% g128-групп `up_proj`, 0.1953125%
`gate_proj` и 1.07421875% `down_proj`. Остальные 58.10546875% `up_proj`,
99.8046875% `gate_proj` и 98.92578125% `down_proj` пока остаются BF16.
...
|---|---:|---:|---:|
| v3 | 0.996572 | 1.000391 | 0.977460 (PyTorch) |
| v4 | 0.996572 | 0.996639 | 0.976159 (Transformers) |
...
1. **NLL budget.** Для условной цели `+5% NLL` и coverage `c` используем
   диагностическую линию `1 + 0.05*c`. При текущем `c=0.03972554` это
   `1.00198628`.
...
```text
wal2/checkpoints/wal-tat-block24_down_after_recovery_s0001-candidate_bf16_mlp.pt
```

- размер: `304,659,816` bytes;
- SHA-256: `fa67c4e8746c3137e73b27062e5aea23a710e04840ddbdd313c84cccf2021050`;
- содержание: полный ternary block 27, Q/K/V/O block 24, 41.89453125%
  `up_proj`, 0.1953125% `gate_proj` и 1.07421875% `down_proj`;
...
1. проверить малый `gate_proj` atom из нового frontier;

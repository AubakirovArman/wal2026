...
| Матрицы в `layer 24` | Q/K/V/O + 45.01953125% up + 0.5859375% gate + 1.46484375% down | 54.98046875% up + 99.4140625% gate + 98.53515625% down |
| Крупные матрицы, включая tied embedding/head | 11 / 197 | 186 |
| Крупные matrix weights | 68,837,376 / 1,720,451,072 | 1,651,613,696 |

Покрытие крупных matrix weights равно `4.001124%`. Нельзя округлять частично
...
Принятые 68,837,376 weights занимают 131.2969 MiB в BF16. После настоящей
Q2-g128 упаковки их расчётный payload составит 17.4379 MiB, экономия —
113.8590 MiB. Экономия VRAM появится только после packed runtime; fake-quant
обучение её не даёт.
...
В `layer 24` приняты 45.01953125% g128-групп `up_proj`, 0.5859375%
`gate_proj` и 1.46484375% `down_proj`. Остальные 54.98046875% `up_proj`,
...
|---|---:|---:|---:|
| v3 | 0.996748 | 1.000469 | 0.978367 (PyTorch) |
| v4 | 0.996748 | 0.996485 | 0.977051 (Transformers) |
...
1. **NLL budget.** Для условной цели `+5% NLL` и coverage `c` используем
   диагностическую линию `1 + 0.05*c`. При текущем `c=0.04001124` это
   `1.00200056`.
...
```text
wal2/checkpoints/wal-tat-block24_gate_after_recovery_s0002-candidate_only.pt
```

- размер: `304,659,052` bytes;
- SHA-256: `170790d8ee49e8342193d58477207a6accd7c39eea8697772767f067448e51de`;
- содержание: полный ternary block 27, Q/K/V/O block 24, 45.01953125%

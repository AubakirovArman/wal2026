...
| Матрицы в `layer 24` | Q/K/V/O + 65.33203125% up + 2.1484375% gate + 4.58984375% down | 34.66796875% up + 97.8515625% gate + 95.41015625% down |
| Крупные матрицы, включая tied embedding/head | 11 / 197 | 186 |
| Крупные matrix weights | 71,983,104 / 1,720,451,072 | 1,648,467,968 |

Покрытие крупных matrix weights равно `4.183967%`. Нельзя округлять частично
...
Принятые 71,983,104 weights занимают 137.2969 MiB в BF16. После настоящей
Q2-g128 упаковки их расчётный payload составит 18.2347 MiB, экономия —
119.0621 MiB. Экономия VRAM появится только после packed runtime; fake-quant
обучение её не даёт.
...
`gate_proj` и 4.58984375% `down_proj`. Остальные 34.66796875% `up_proj`,
97.8515625% `gate_proj` и 95.41015625% `down_proj` пока остаются BF16.
...
|---|---:|---:|---:|
| v3 | 0.996626 | 1.000363 | 0.979853 (PyTorch) |
| v4 | 0.996626 | 0.996791 | 0.979539 (Transformers) |
...
1. **NLL budget.** Для условной цели `+5% NLL` и coverage `c` используем
   диагностическую линию `1 + 0.05*c`. При текущем `c=0.04183967` это
   `1.00209198`.
...
```text
wal2/checkpoints/wal-tat-block24_up_headroom_growth_s0015-candidate_only.pt
```

- размер: `304,658,813` bytes;
- SHA-256: `53effc035fae468e6087f45e044e43b4551448fdd1c28fd4363be03b9507d797`;
- содержание: полный ternary block 27, Q/K/V/O block 24, 65.33203125%
  `up_proj`, 2.1484375% `gate_proj` и 4.58984375% `down_proj`;

...
| Матрицы в `layer 24` | Q/K/V/O + 45.01953125% up + 0.390625% gate + 1.46484375% down | 54.98046875% up + 99.609375% gate + 98.53515625% down |
| Крупные матрицы, включая tied embedding/head | 11 / 197 | 186 |
| Крупные matrix weights | 68,812,800 / 1,720,451,072 | 1,651,638,272 |

Покрытие крупных matrix weights равно `3.999695%`. Нельзя округлять частично
...
Принятые 68,812,800 weights занимают 131.2500 MiB в BF16. После настоящей
Q2-g128 упаковки их расчётный payload составит 17.4316 MiB, экономия —
113.8184 MiB. Экономия VRAM появится только после packed runtime; fake-quant
обучение её не даёт.
...
В `layer 24` приняты 45.01953125% g128-групп `up_proj`, 0.390625%
`gate_proj` и 1.46484375% `down_proj`. Остальные 54.98046875% `up_proj`,
99.609375% `gate_proj` и 98.53515625% `down_proj` пока остаются BF16.
...
|---|---:|---:|---:|
| v3 | 0.996764 | 1.000459 | 0.978365 (PyTorch) |
| v4 | 0.996764 | 0.996486 | 0.977022 (Transformers) |
...
1. **NLL budget.** Для условной цели `+5% NLL` и coverage `c` используем
   диагностическую линию `1 + 0.05*c`. При текущем `c=0.03999695` это
   `1.00199985`.
...
```text
wal2/checkpoints/wal-tat-block24_down_after_recovery_s0002-candidate_bf16_mlp.pt
```

- размер: `304,659,816` bytes;
- SHA-256: `58e4ac640c72570c67d7d391561c789fd29326c18d8fe84b58aaa35cbfbc4838`;
- содержание: полный ternary block 27, Q/K/V/O block 24, 45.01953125%
  `up_proj`, 0.390625% `gate_proj` и 1.46484375% `down_proj`;

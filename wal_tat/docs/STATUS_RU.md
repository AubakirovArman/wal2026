...
| Матрицы в `layer 24` | Q/K/V/O + 51.26953125% up + 0.78125% gate + 2.24609375% down | 48.73046875% up + 99.21875% gate + 97.75390625% down |
| Крупные матрицы, включая tied embedding/head | 11 / 197 | 186 |
| Крупные matrix weights | 69,746,688 / 1,720,451,072 | 1,650,704,384 |

Покрытие крупных matrix weights равно `4.053977%`. Нельзя округлять частично
...
Принятые 69,746,688 weights занимают 133.0313 MiB в BF16. После настоящей
Q2-g128 упаковки их расчётный payload составит 17.6682 MiB, экономия —
115.3630 MiB. Экономия VRAM появится только после packed runtime; fake-quant
обучение её не даёт.
...
В `layer 24` приняты 51.26953125% g128-групп `up_proj`, 0.78125%
`gate_proj` и 2.24609375% `down_proj`. Остальные 48.73046875% `up_proj`,
99.21875% `gate_proj` и 97.75390625% `down_proj` пока остаются BF16.
...
|---|---:|---:|---:|
| v3 | 0.997037 | 1.001011 | 0.979295 (PyTorch) |
| v4 | 0.997037 | 0.996873 | 0.977916 (Transformers) |
...
1. **NLL budget.** Для условной цели `+5% NLL` и coverage `c` используем
   диагностическую линию `1 + 0.05*c`. При текущем `c=0.04053977` это
   `1.00202699`.
...
```text
wal2/checkpoints/wal-tat-block24_down_after_recovery_s0004-candidate_bf16_mlp.pt
```

- размер: `304,659,816` bytes;
- SHA-256: `fe7a494768bfd88d1b574b12e3545604c5b16cafddeae62118c55e91d28a01ef`;
- содержание: полный ternary block 27, Q/K/V/O block 24, 51.26953125%
  `up_proj`, 0.78125% `gate_proj` и 2.24609375% `down_proj`;
...
1. выполнить `gate_proj +0.1953125%` matched-транзакцию;
2. провести masked proxy recovery частичного MLP и повторить оба holdout;

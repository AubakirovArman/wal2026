...
| Матрицы в `layer 24` | Q/K/V/O + 45.01953125% up + 0.390625% gate + 1.07421875% down | 54.98046875% up + 99.609375% gate + 98.92578125% down |
| Крупные матрицы, включая tied embedding/head | 11 / 197 | 186 |
| Крупные matrix weights | 68,763,648 / 1,720,451,072 | 1,651,687,424 |

Покрытие крупных matrix weights равно `3.996838%`. Нельзя округлять частично
...
Принятые 68,763,648 weights занимают 131.1562 MiB в BF16. После настоящей
Q2-g128 упаковки их расчётный payload составит 17.4192 MiB, экономия —
113.7371 MiB. Экономия VRAM появится только после packed runtime; fake-quant
обучение её не даёт.
...
`gate_proj` и 1.07421875% `down_proj`. Остальные 58.10546875% `up_proj`,
99.609375% `gate_proj` и 98.92578125% `down_proj` пока остаются BF16.
...
|---|---:|---:|---:|
| v3 | 0.996732 | 1.000441 | 0.978275 (PyTorch) |
| v4 | 0.996732 | 0.996494 | 0.977081 (Transformers) |
...
1. **NLL budget.** Для условной цели `+5% NLL` и coverage `c` используем
   диагностическую линию `1 + 0.05*c`. При текущем `c=0.03996838` это
   `1.00199842`.
...
```text
wal2/checkpoints/wal-tat-block24_up_headroom_growth_s0005-candidate_bf16_mlp.pt
```

- размер: `304,659,641` bytes;
- SHA-256: `7d96b4b985c6bb7924e126b5935af40894011240a13d5c133f5464eebfae4b79`;
- содержание: полный ternary block 27, Q/K/V/O block 24, 45.01953125%
  `up_proj`, 0.390625% `gate_proj` и 1.07421875% `down_proj`;

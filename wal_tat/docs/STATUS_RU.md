...
| Матрицы в `layer 24` | Q/K/V/O + 32.51953125% up + 0.1953125% gate + 0.09765625% down | 67.48046875% up + 99.8046875% gate + 99.90234375% down |
| Крупные матрицы, включая tied embedding/head | 11 / 197 | 186 |
| Крупные matrix weights | 67,043,328 / 1,720,451,072 | 1,653,407,744 |

Покрытие крупных matrix weights равно `3.896846%`. Нельзя округлять частично
...
Принятые 67,043,328 weights занимают 127.8750 MiB в BF16. После настоящей
Q2-g128 упаковки их расчётный payload составит 16.9834 MiB, экономия —
110.8916 MiB. Экономия VRAM появится только после packed runtime; fake-quant
обучение её не даёт.
...
`gate_proj` и 0.09765625% `down_proj`. Остальные 67.48046875% `up_proj`,
99.8046875% `gate_proj` и 99.90234375% `down_proj` пока остаются BF16.
...
|---|---:|---:|---:|
| v3 | 0.997550 | 1.001528 | 0.978629 (PyTorch) |
| v4 | 0.997550 | 0.997836 | 0.976648 (Transformers) |
...
1. **NLL budget.** Для условной цели `+5% NLL` и coverage `c` используем
   диагностическую линию `1 + 0.05*c`. При текущем `c=0.03896846` это
   `1.00194842`.
...
заранее объявленной независимой метрике.

## Лучший воспроизводимый checkpoint

```text
wal2/checkpoints/wal-tat-block24_down_auto_s0001-candidate_only.pt
```

- размер: `304,657,174` bytes;
- SHA-256: `1289304fec2f1b8ae92022d877feb679f8e861da87786c47c41b6b26db7724f8`;
- содержание: полный ternary block 27, Q/K/V/O block 24, 32.51953125%
  `up_proj`, 0.1953125% `gate_proj` и 0.09765625% `down_proj`;
...
1. продолжить автоматические `gate_proj/down_proj` кампании малыми атомами;
2. проверить более крупный `down_proj` атом отдельной matched-ablation, не
   ослабляя coverage-aware gate;

...
| Матрицы в `layer 24` | Q/K/V/O + 41.89453125% up + 0.390625% gate + 1.07421875% down | 58.10546875% up + 99.609375% gate + 98.92578125% down |
| Крупные матрицы, включая tied embedding/head | 11 / 197 | 186 |
| Крупные matrix weights | 68,370,432 / 1,720,451,072 | 1,652,080,640 |

Покрытие крупных matrix weights равно `3.973983%`. Нельзя округлять частично
...
Принятые 68,370,432 weights занимают 130.4062 MiB в BF16. После настоящей
Q2-g128 упаковки их расчётный payload составит 17.3196 MiB, экономия —
113.0867 MiB. Экономия VRAM появится только после packed runtime; fake-quant
обучение её не даёт.
...
В `layer 24` приняты 41.89453125% g128-групп `up_proj`, 0.390625%
...
|---|---:|---:|---:|
| v3 | 0.996649 | 1.000350 | 0.977673 (PyTorch) |
| v4 | 0.996649 | 0.996597 | 0.976294 (Transformers) |
...
1. **NLL budget.** Для условной цели `+5% NLL` и coverage `c` используем
   диагностическую линию `1 + 0.05*c`. При текущем `c=0.03973983` это
   `1.00198699`.
...
```text
wal2/checkpoints/wal-tat-block24_gate_after_recovery_s0001-candidate_bf16_mlp.pt
```

- размер: `304,659,816` bytes;
- SHA-256: `6e7e554fa71bc20f6de54b993c0d153382325191aa9c0b7660168552821aba0a`;

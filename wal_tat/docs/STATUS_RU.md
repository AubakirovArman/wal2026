...
| Матрицы в `layer 24` | Q/K/V/O + 74.12109375% up + 4.39453125% gate + 8.7890625% down | 25.87890625% up + 95.60546875% gate + 91.2109375% down |
| Крупные матрицы, включая tied embedding/head | 11 / 197 | 186 |
| Крупные matrix weights | 73,900,032 / 1,720,451,072 | 1,646,551,040 |

Покрытие крупных matrix weights равно `4.295387%`. Нельзя округлять частично
...
Принятые 73,900,032 weights занимают 140.9531 MiB в BF16. После настоящей
Q2-g128 упаковки их расчётный payload составит 18.7203 MiB, экономия —
122.2328 MiB. Экономия VRAM появится только после packed runtime; fake-quant
обучение её не даёт.
...
В `layer 24` приняты 74.12109375% g128-групп `up_proj`, 4.39453125%
`gate_proj` и 8.7890625% `down_proj`. Остальные 25.87890625% `up_proj`,
95.60546875% `gate_proj` и 91.2109375% `down_proj` пока остаются BF16.
...
|---|---:|---:|---:|
| v3 | 0.997184 | 1.001664 | 0.981453 (PyTorch) |
| v4 | 0.997184 | 0.997847 | 0.981939 (Transformers) |
...
1. **NLL budget.** Для условной цели `+5% NLL` и coverage `c` используем
   диагностическую линию `1 + 0.05*c`. При текущем `c=0.04295387` это
   `1.00214769`.
...
```text
wal2/checkpoints/wal-tat-block24_gate_after_recovery_s0022-candidate_only.pt
```

- размер: `304,659,052` bytes;
- SHA-256: `f61762fcefef7bac94b3cd9f2dbb1072087904500a109a2f66ef0145a2035215`;
- содержание: полный ternary block 27, Q/K/V/O block 24, 74.12109375%
  `up_proj`, 4.39453125% `gate_proj` и 8.7890625% `down_proj`;
...
1. выполнить coverage-neutral masked proxy recovery v7 на текущем frontier;
2. подтвердить неизменность masks и непринятых BF16 weights двумя audit;
3. продолжить минимальным `up_proj +0.09765625%` либо сделать rollback;

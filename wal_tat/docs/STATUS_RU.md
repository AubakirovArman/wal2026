...
| Матрицы в `layer 24` | Q/K/V/O + 67.67578125% up + 2.734375% gate + 5.76171875% down | 32.32421875% up + 97.265625% gate + 94.23828125% down |
| Крупные матрицы, включая tied embedding/head | 11 / 197 | 186 |
| Крупные matrix weights | 72,499,200 / 1,720,451,072 | 1,647,951,872 |

Покрытие крупных matrix weights равно `4.213965%`. Нельзя округлять частично
...
Принятые 72,499,200 weights занимают 138.2812 MiB в BF16. После настоящей
Q2-g128 упаковки их расчётный payload составит 18.3655 MiB, экономия —
119.9158 MiB. Экономия VRAM появится только после packed runtime; fake-quant
обучение её не даёт.
...
В `layer 24` приняты 67.67578125% g128-групп `up_proj`, 2.734375%
...
|---|---:|---:|---:|
| v3 | 0.996872 | 1.000730 | 0.980559 (PyTorch) |
| v4 | 0.996872 | 0.997090 | 0.980265 (Transformers) |
...
1. **NLL budget.** Для условной цели `+5% NLL` и coverage `c` используем
   диагностическую линию `1 + 0.05*c`. При текущем `c=0.04213965` это
   `1.00210698`.
...
```text
wal2/checkpoints/wal-tat-block24_gate_after_recovery_s0013-candidate_only.pt
```

- размер: `304,659,052` bytes;
- SHA-256: `e909e6d2109a4fa796bce27007532b5ea17332c87b254cac5114e5b426732218`;

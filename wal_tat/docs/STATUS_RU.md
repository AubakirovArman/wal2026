...
| Матрицы в `layer 24` | Q/K/V/O + 69.23828125% up + 3.125% gate + 6.54296875% down | 30.76171875% up + 96.875% gate + 93.45703125% down |
| Крупные матрицы, включая tied embedding/head | 11 / 197 | 186 |
| Крупные matrix weights | 72,843,264 / 1,720,451,072 | 1,647,607,808 |

Покрытие крупных matrix weights равно `4.233963%`. Нельзя округлять частично
...
Принятые 72,843,264 weights занимают 138.9375 MiB в BF16. После настоящей
Q2-g128 упаковки их расчётный payload составит 18.4526 MiB, экономия —
120.4849 MiB. Экономия VRAM появится только после packed runtime; fake-quant
обучение её не даёт.
...
В `layer 24` приняты 69.23828125% g128-групп `up_proj`, 3.125%
...
1. **NLL budget.** Для условной цели `+5% NLL` и coverage `c` используем
   диагностическую линию `1 + 0.05*c`. При текущем `c=0.04233963` это
   `1.00211698`.

...
| Матрицы в `layer 24` | Q/K/V/O + 63.76953125% up + 1.953125% gate + 4.19921875% down | 36.23046875% up + 98.046875% gate + 95.80078125% down |
| Крупные матрицы, включая tied embedding/head | 11 / 197 | 186 |
| Крупные matrix weights | 71,712,768 / 1,720,451,072 | 1,648,738,304 |

Покрытие крупных matrix weights равно `4.168254%`. Нельзя округлять частично
...
Принятые 71,712,768 weights занимают 136.7812 MiB в BF16. После настоящей
Q2-g128 упаковки их расчётный payload составит 18.1663 MiB, экономия —
118.6150 MiB. Экономия VRAM появится только после packed runtime; fake-quant
обучение её не даёт.
...
`gate_proj` и 4.19921875% `down_proj`. Остальные 36.23046875% `up_proj`,
98.046875% `gate_proj` и 95.80078125% `down_proj` пока остаются BF16.
...
|---|---:|---:|---:|
| v3 | 0.996470 | 1.000192 | 0.979820 (PyTorch) |
| v4 | 0.996470 | 0.996487 | 0.979049 (Transformers) |
...
1. **NLL budget.** Для условной цели `+5% NLL` и coverage `c` используем
   диагностическую линию `1 + 0.05*c`. При текущем `c=0.04168254` это
   `1.00208413`.
...
```text
wal2/checkpoints/wal-tat-block24_up_headroom_growth_s0014-candidate_bf16_mlp.pt
```

- размер: `304,659,641` bytes;
- SHA-256: `1376d964458de2f040309774a44e5e2cc6d437c782d31621f42686262abbcd66`;

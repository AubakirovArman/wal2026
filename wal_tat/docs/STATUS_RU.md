...
| Матрицы в `layer 24` | Q/K/V/O + 48.14453125% up + 0.78125% gate + 1.85546875% down | 51.85546875% up + 99.21875% gate + 98.14453125% down |
| Крупные матрицы, включая tied embedding/head | 11 / 197 | 186 |
| Крупные matrix weights | 69,304,320 / 1,720,451,072 | 1,651,146,752 |

Покрытие крупных matrix weights равно `4.028265%`. Нельзя округлять частично
...
Принятые 69,304,320 weights занимают 132.1875 MiB в BF16. После настоящей
Q2-g128 упаковки их расчётный payload составит 17.5562 MiB, экономия —
114.6313 MiB. Экономия VRAM появится только после packed runtime; fake-quant
обучение её не даёт.
...
В `layer 24` приняты 48.14453125% g128-групп `up_proj`, 0.78125%
`gate_proj` и 1.85546875% `down_proj`. Остальные 51.85546875% `up_proj`,
99.21875% `gate_proj` и 98.14453125% `down_proj` пока остаются BF16.
...
|---|---:|---:|---:|
| v3 | 0.996887 | 1.000643 | 0.978844 (PyTorch) |
| v4 | 0.996887 | 0.996653 | 0.977507 (Transformers) |
...
1. **NLL budget.** Для условной цели `+5% NLL` и coverage `c` используем
   диагностическую линию `1 + 0.05*c`. При текущем `c=0.04028265` это
   `1.00201413`.
...
```text
wal2/checkpoints/wal-tat-block24_gate_after_recovery_s0003-candidate_bf16_mlp.pt
```

- размер: `304,659,816` bytes;
- SHA-256: `44b878d17ad77f6d7030d389071b4f950f9d7d8118f8cc6b8e7b4b775ddda130`;
- содержание: полный ternary block 27, Q/K/V/O block 24, 48.14453125%
  `up_proj`, 0.78125% `gate_proj` и 1.85546875% `down_proj`;

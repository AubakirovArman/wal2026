...
| Матрицы в `layer 24` | Q/K/V/O + 63.76953125% up + 1.953125% gate + 4.58984375% down | 36.23046875% up + 98.046875% gate + 95.41015625% down |
| Крупные матрицы, включая tied embedding/head | 11 / 197 | 186 |
| Крупные matrix weights | 71,761,920 / 1,720,451,072 | 1,648,689,152 |

Покрытие крупных matrix weights равно `4.171111%`. Нельзя округлять частично
...
Принятые 71,761,920 weights занимают 136.8750 MiB в BF16. После настоящей
Q2-g128 упаковки их расчётный payload составит 18.1787 MiB, экономия —
118.6963 MiB. Экономия VRAM появится только после packed runtime; fake-quant
обучение её не даёт.
...
В `layer 24` приняты 63.76953125% g128-групп `up_proj`, 1.953125%
`gate_proj` и 4.58984375% `down_proj`. Остальные 36.23046875% `up_proj`,
98.046875% `gate_proj` и 95.41015625% `down_proj` пока остаются BF16.
...
|---|---:|---:|---:|
| v3 | 0.996496 | 1.000247 | 0.979796 (PyTorch) |
| v4 | 0.996496 | 0.996467 | 0.979181 (Transformers) |
...
1. **NLL budget.** Для условной цели `+5% NLL` и coverage `c` используем
   диагностическую линию `1 + 0.05*c`. При текущем `c=0.04171111` это
   `1.00208556`.
...
```text
wal2/checkpoints/wal-tat-block24_down_after_recovery_s0010-candidate_only.pt
```

- размер: `304,659,052` bytes;
- SHA-256: `972a2db558f6c269154874cbd130955b3118a2caffe01d618e1d3bff22b043e7`;
- содержание: полный ternary block 27, Q/K/V/O block 24, 63.76953125%
  `up_proj`, 1.953125% `gate_proj` и 4.58984375% `down_proj`;

...
| Матрицы в `layer 24` | Q/K/V/O + 65.33203125% up + 2.1484375% gate + 4.98046875% down | 34.66796875% up + 97.8515625% gate + 95.01953125% down |
| Крупные матрицы, включая tied embedding/head | 11 / 197 | 186 |
| Крупные matrix weights | 72,032,256 / 1,720,451,072 | 1,648,418,816 |

Покрытие крупных matrix weights равно `4.186824%`. Нельзя округлять частично
...
Принятые 72,032,256 weights занимают 137.3906 MiB в BF16. После настоящей
Q2-g128 упаковки их расчётный payload составит 18.2472 MiB, экономия —
119.1434 MiB. Экономия VRAM появится только после packed runtime; fake-quant
обучение её не даёт.
...
В `layer 24` приняты 65.33203125% g128-групп `up_proj`, 2.1484375%
`gate_proj` и 4.98046875% `down_proj`. Остальные 34.66796875% `up_proj`,
97.8515625% `gate_proj` и 95.01953125% `down_proj` пока остаются BF16.
...
|---|---:|---:|---:|
| v3 | 0.996633 | 1.000443 | 0.979897 (PyTorch) |
| v4 | 0.996633 | 0.996784 | 0.979511 (Transformers) |
...
1. **NLL budget.** Для условной цели `+5% NLL` и coverage `c` используем
   диагностическую линию `1 + 0.05*c`. При текущем `c=0.04186824` это
   `1.00209341`.
...
```text
wal2/checkpoints/wal-tat-block24_down_after_recovery_s0011-candidate_only.pt
```

- размер: `304,659,052` bytes;
- SHA-256: `d4fda099ad50dbceb4b24e137368bb1c14d160f6712f8dd22550983187b2c2ec`;
- содержание: полный ternary block 27, Q/K/V/O block 24, 65.33203125%
  `up_proj`, 2.1484375% `gate_proj` и 4.98046875% `down_proj`;

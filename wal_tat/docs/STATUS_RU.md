...
| Матрицы в `layer 24` | Q/K/V/O + 38.76953125% up + 0.1953125% gate + 0.68359375% down | 61.23046875% up + 99.8046875% gate + 99.31640625% down |
| Крупные матрицы, включая tied embedding/head | 11 / 197 | 186 |
| Крупные matrix weights | 67,903,488 / 1,720,451,072 | 1,652,547,584 |

Покрытие крупных matrix weights равно `3.946842%`. Нельзя округлять частично
...
Принятые 67,903,488 weights занимают 129.5156 MiB в BF16. После настоящей
Q2-g128 упаковки их расчётный payload составит 17.2013 MiB, экономия —
112.3143 MiB. Экономия VRAM появится только после packed runtime; fake-quant
обучение её не даёт.
...
В `layer 24` приняты 38.76953125% g128-групп `up_proj`, 0.1953125%
`gate_proj` и 0.68359375% `down_proj`. Остальные 61.23046875% `up_proj`,
...
|---|---:|---:|---:|
| v3 | 0.997068 | 1.000755 | 0.978374 (PyTorch) |
| v4 | 0.997068 | 0.997046 | 0.976825 (Transformers) |
...
1. **NLL budget.** Для условной цели `+5% NLL` и coverage `c` используем
   диагностическую линию `1 + 0.05*c`. При текущем `c=0.03946842` это
   `1.00197342`.
...
`38.76953125%`, то есть крупный шаг воспроизвёлся два раза подряд.

## Лучший воспроизводимый checkpoint

```text
wal2/checkpoints/wal-tat-block24_up_headroom_growth_s0003-candidate_bf16_mlp.pt
```

- размер: `304,659,641` bytes;
- SHA-256: `3394d8e703b8d1a57ac2ac728c31385187e3dd23e45f4664177254b54d84ef9e`;
- содержание: полный ternary block 27, Q/K/V/O block 24, 38.76953125%

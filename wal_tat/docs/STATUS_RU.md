...
| Q4-g128 weights | 0 | 66,786,944 (`3.881944%`) |
| Q8-g128 weights | 0 | 105,444,864 (`6.128908%`) |
...
`layer 25` полностью low-bit: `7.999929%` Q4 и `92.000071%` Q8 при
`7.805003 bpw`. Fresh development прошёл с ratios
`0.994942 / 1.001131 / 1.019953`; sealed audit-v31 —
`0.993532 / 1.003477 / 0.971895`, incremental worst против immutable `s0053`
`1.004159 <= 1.005`. Высокая Q8-доля является безопасным верхним bound и
будет обратно сжиматься после фиксации coverage.
...
79,426,432 Q2-весов, 66,786,944 Q4-весов и 105,444,864 Q8-весов. Текущий
579,594,601-byte `.pt` хранит дублирующиеся int8 training arrays и не является
packed deploy-файлом. Расчётный packed payload этих low-bit весов —
155.093 MiB против 480 MiB в BF16; реальная экономия VRAM появится только
...
`model.layers.25` полностью low-bit: 4,026,496 весов используют Q4 и
46,305,152 — Q8. BF16 в семи крупных матрицах блока нет.
...
Он прошёл fresh development и sealed audit-v31. Ни Q4/Q8-веса, ни mixed

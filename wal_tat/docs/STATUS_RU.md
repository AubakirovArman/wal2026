...
| Матрицы в `layer 24` | Q/K/V/O + 48.14453125% up + 0.5859375% gate + 1.85546875% down | 51.85546875% up + 99.4140625% gate + 98.14453125% down |
| Крупные матрицы, включая tied embedding/head | 11 / 197 | 186 |
| Крупные matrix weights | 69,279,744 / 1,720,451,072 | 1,651,171,328 |

Покрытие крупных matrix weights равно `4.026836%`. Нельзя округлять частично
...
Принятые 69,279,744 weights занимают 132.1406 MiB в BF16. После настоящей
Q2-g128 упаковки их расчётный payload составит 17.5499 MiB, экономия —
114.5907 MiB. Экономия VRAM появится только после packed runtime; fake-quant
обучение её не даёт.
...
В `layer 24` приняты 48.14453125% g128-групп `up_proj`, 0.5859375%
`gate_proj` и 1.85546875% `down_proj`. Остальные 51.85546875% `up_proj`,
99.4140625% `gate_proj` и 98.14453125% `down_proj` пока остаются BF16.
...
|---|---:|---:|---:|
| v3 | 0.996852 | 1.000677 | 0.978720 (PyTorch) |
| v4 | 0.996852 | 0.996764 | 0.977462 (Transformers) |
...
1. **NLL budget.** Для условной цели `+5% NLL` и coverage `c` используем
   диагностическую линию `1 + 0.05*c`. При текущем `c=0.04026836` это
   `1.00201342`.
...
```text
wal2/checkpoints/wal-tat-block24_down_after_recovery_s0003-candidate_bf16_mlp.pt
```

- размер: `304,659,816` bytes;
- SHA-256: `62f5d729918379954ef5d4e74f6962051ab7e2e4f37b0d0be571c1f637dc16f3`;
- содержание: полный ternary block 27, Q/K/V/O block 24, 48.14453125%
  `up_proj`, 0.5859375% `gate_proj` и 1.85546875% `down_proj`;
...
1. продолжить matched round-robin: `up`, затем `down`, затем `gate`;

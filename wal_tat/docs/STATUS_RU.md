...
| Матрицы в `layer 24` | Q/K/V/O + 32.51953125% up + 0.1953125% gate + 0.48828125% down | 67.48046875% up + 99.8046875% gate + 99.51171875% down |
| Крупные матрицы, включая tied embedding/head | 11 / 197 | 186 |
| Крупные matrix weights | 67,092,480 / 1,720,451,072 | 1,653,358,592 |

Покрытие крупных matrix weights равно `3.899703%`. Нельзя округлять частично
...
Принятые 67,092,480 weights занимают 127.9688 MiB в BF16. После настоящей
Q2-g128 упаковки их расчётный payload составит 16.9958 MiB, экономия —
110.9729 MiB. Экономия VRAM появится только после packed runtime; fake-quant
обучение её не даёт.
...
`gate_proj` и 0.48828125% `down_proj`. Остальные 67.48046875% `up_proj`,
99.8046875% `gate_proj` и 99.51171875% `down_proj` пока остаются BF16.
...
|---|---:|---:|---:|
| v3 | 0.997562 | 1.001488 | 0.978640 (PyTorch) |
| v4 | 0.997562 | 0.997924 | 0.976721 (Transformers) |
...
1. **NLL budget.** Для условной цели `+5% NLL` и coverage `c` используем
   диагностическую линию `1 + 0.05*c`. При текущем `c=0.03899703` это
   `1.00194985`.
...
узкого margin уменьшил предлагаемый следующий атом до `0.1953125%`.

## Лучший воспроизводимый checkpoint

```text
wal2/checkpoints/wal-tat-block24_down_growth_s0001-candidate_bf16_mlp.pt
```

- размер: `304,658,224` bytes;
- SHA-256: `cdd33bbf09384828aef48cde1d21d6fed7c13d86526b91612bd52e99f8b2432b`;
- содержание: полный ternary block 27, Q/K/V/O block 24, 32.51953125%
  `up_proj`, 0.1953125% `gate_proj` и 0.48828125% `down_proj`;
...
1. продолжить `down_proj` атомом `0.1953125%`, предложенным контроллером;

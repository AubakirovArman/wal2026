...
| Матрицы в `layer 24` | Q/K/V/O + 48.14453125% up + 0.5859375% gate + 1.46484375% down | 51.85546875% up + 99.4140625% gate + 98.53515625% down |
| Крупные матрицы, включая tied embedding/head | 11 / 197 | 186 |
| Крупные matrix weights | 69,230,592 / 1,720,451,072 | 1,651,220,480 |

Покрытие крупных matrix weights равно `4.023979%`. Нельзя округлять частично
...
Принятые 69,230,592 weights занимают 132.0469 MiB в BF16. После настоящей
Q2-g128 упаковки их расчётный payload составит 17.5375 MiB, экономия —
114.5094 MiB. Экономия VRAM появится только после packed runtime; fake-quant
обучение её не даёт.
...
`gate_proj` и 1.46484375% `down_proj`. Остальные 51.85546875% `up_proj`,
99.4140625% `gate_proj` и 98.53515625% `down_proj` пока остаются BF16.
...
|---|---:|---:|---:|
| v3 | 0.996842 | 1.000692 | 0.978706 (PyTorch) |
| v4 | 0.996842 | 0.996730 | 0.977443 (Transformers) |
...
1. **NLL budget.** Для условной цели `+5% NLL` и coverage `c` используем
   диагностическую линию `1 + 0.05*c`. При текущем `c=0.04023979` это
   `1.00201199`.
...
```text
wal2/checkpoints/wal-tat-block24_up_headroom_growth_s0006-candidate_bf16_mlp.pt
```

- размер: `304,659,641` bytes;
- SHA-256: `23eb2408fcbf18bc066ac74f1972794a29cfb205f2bdcfede1fdbdd2a3d48281`;
- содержание: полный ternary block 27, Q/K/V/O block 24, 48.14453125%
  `up_proj`, 0.5859375% `gate_proj` и 1.46484375% `down_proj`;

...
| Матрицы в `layer 24` | Q/K/V/O + 28.125% `up_proj` | 71.875% up + down/gate |
| Крупные матрицы, включая tied embedding/head | 11 / 197 | 186 |
| Крупные matrix weights | 66,453,504 / 1,720,451,072 | 1,653,997,568 |

Покрытие крупных matrix weights равно `3.862563%`. Нельзя округлять частично
...
Принятые 66,453,504 weights занимают 126.75 MiB в BF16. После настоящей
Q2-g128 упаковки их расчётный payload составит 16.8340 MiB, экономия —
109.9160 MiB. Экономия VRAM появится только после packed runtime; fake-quant
обучение её не даёт.
...
В `layer 24 up_proj` независимо приняты 28.125% g128-групп. Остальные 71.875%
...
|---|---:|---:|---:|
| v3 | 0.997582 | 1.001603 | 0.978089 (PyTorch) |
| v4 | 0.997582 | 0.998143 | 0.976273 (Transformers) |
...
1. **NLL budget.** Для условной цели `+5% NLL` и coverage `c` используем
   диагностическую линию `1 + 0.05*c`. При текущем `c=0.03862563` это
   `1.00193128`.
...
принятый coverage `up_proj` теперь 28.125%. Попытка добавить 12.5% за один
...
```text
wal2/checkpoints/wal-tat-block24_up_incremental_28p125_v10-candidate_only.pt
```

- размер: `266,613,936` bytes;
- SHA-256: `7ca69950016af5f38864ca04a30b17d8ef793acad1516593e4e24a64f4752bf7`;
- содержание: полный ternary block 27, Q/K/V/O block 24 и 28.125% его `up_proj`;
...
1. автоматизировать уменьшение/повтор атомов по запасу holdout gate;
2. продолжить `layer 24 up_proj` атомами не больше 0.78125%;
3. добавить непрерывную BF16-компенсацию `down/gate`, не коммитя их раньше
   собственного ternary gate;

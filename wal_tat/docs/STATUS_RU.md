...
| Матрицы в `layer 24` | Q/K/V/O + 74.90234375% up + 5.17578125% gate + 9.47265625% down | 25.09765625% up + 94.82421875% gate + 90.52734375% down |
| Крупные матрицы, включая tied embedding/head | 11 / 197 | 186 |
| Крупные matrix weights | 74,182,656 / 1,720,451,072 | 1,646,268,416 |

Покрытие крупных matrix weights равно `4.311814%`. Нельзя округлять частично
...
Принятые 74,182,656 weights занимают 141.4922 MiB в BF16. После настоящей
Q2-g128 упаковки их расчётный payload составит 18.7919 MiB, экономия —
122.7003 MiB. Экономия VRAM появится только после packed runtime; fake-quant
обучение её не даёт.
...
В `layer 24` приняты 74.90234375% g128-групп `up_proj`, 5.17578125%
`gate_proj` и 9.47265625% `down_proj`. Остальные 25.09765625% `up_proj`,
94.82421875% `gate_proj` и 90.52734375% `down_proj` пока остаются BF16.
...
|---|---:|---:|---:|
| v3 | 0.997033 | 1.001280 | 0.981543 (PyTorch) |
| v4 | 0.997033 | 0.997698 | 0.982167 (Transformers) |
...
```text
wal2/checkpoints/wal-tat-block24_up_headroom_growth_s0038-candidate_only.pt
```

- размер: `304,658,813` bytes;
- SHA-256: `3bf1f3e255a5d1f8c35a05af64a6484fa1e0b5508cee13226c50fb5bff3c0c07`;
- содержание: полный ternary block 27, Q/K/V/O block 24, 74.90234375%
  `up_proj`, 5.17578125% `gate_proj` и 9.47265625% `down_proj`;
...
1. продолжить `down_proj s0031 +0.09765625%` либо сделать rollback;
2. затем проверить `gate_proj s0030 +0.1953125%` и вернуться к `up_proj`;

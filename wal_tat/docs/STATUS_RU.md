...
| Матрицы в `layer 24` | Q/K/V/O + 74.4140625% up + 4.58984375% gate + 8.984375% down | 25.5859375% up + 95.41015625% gate + 91.015625% down |
| Крупные матрицы, включая tied embedding/head | 11 / 197 | 186 |
| Крупные matrix weights | 73,986,048 / 1,720,451,072 | 1,646,465,024 |

Покрытие крупных matrix weights равно `4.300387%`. Нельзя округлять частично
...
Принятые 73,986,048 weights занимают 141.1172 MiB в BF16. После настоящей
Q2-g128 упаковки их расчётный payload составит 18.7421 MiB, экономия —
122.3751 MiB. Экономия VRAM появится только после packed runtime; fake-quant
обучение её не даёт.
...
В `layer 24` приняты 74.4140625% g128-групп `up_proj`, 4.58984375%
`gate_proj` и 8.984375% `down_proj`. Остальные 25.5859375% `up_proj`,
95.41015625% `gate_proj` и 91.015625% `down_proj` пока остаются BF16.
...
|---|---:|---:|---:|
| v3 | 0.996734 | 1.001148 | 0.980426 (PyTorch) |
| v4 | 0.996734 | 0.997362 | 0.980878 (Transformers) |

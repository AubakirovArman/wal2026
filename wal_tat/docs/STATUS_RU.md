...
| Матрицы в `layer 24` | Q/K/V/O + 60.64453125% up + 1.5625% gate + 3.80859375% down | 39.35546875% up + 98.4375% gate + 96.19140625% down |
| Крупные матрицы, включая tied embedding/head | 11 / 197 | 186 |
| Крупные matrix weights | 71,221,248 / 1,720,451,072 | 1,649,229,824 |

Покрытие крупных matrix weights равно `4.139685%`. Нельзя округлять частично
...
Принятые 71,221,248 weights занимают 135.8438 MiB в BF16. После настоящей
Q2-g128 упаковки их расчётный payload составит 18.0417 MiB, экономия —
117.8020 MiB. Экономия VRAM появится только после packed runtime; fake-quant
обучение её не даёт.
...
В `layer 24` приняты 60.64453125% g128-групп `up_proj`, 1.5625%
`gate_proj` и 3.80859375% `down_proj`. Остальные 39.35546875% `up_proj`,
98.4375% `gate_proj` и 96.19140625% `down_proj` пока остаются BF16.
...
|---|---:|---:|---:|
| v3 | 0.996747 | 1.000555 | 0.979864 (PyTorch) |
| v4 | 0.996747 | 0.996642 | 0.978850 (Transformers) |
...
```text
wal2/checkpoints/wal-tat-block24_down_after_recovery_s0008-candidate_bf16_mlp.pt
```

- размер: `304,659,816` bytes;
- SHA-256: `c455935d8a69c5761173df401c00c5e2f40819e8a6ae698987bca52b8ae443ac`;
- содержание: полный ternary block 27, Q/K/V/O block 24, 60.64453125%
  `up_proj`, 1.5625% `gate_proj` и 3.80859375% `down_proj`;

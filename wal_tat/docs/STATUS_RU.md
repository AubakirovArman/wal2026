...
| Матрицы в `layer 24` | Q/K/V/O + 65.33203125% up + 2.34375% gate + 4.98046875% down | 34.66796875% up + 97.65625% gate + 95.01953125% down |
| Крупные матрицы, включая tied embedding/head | 11 / 197 | 186 |
| Крупные matrix weights | 72,056,832 / 1,720,451,072 | 1,648,394,240 |

Покрытие крупных matrix weights равно `4.188252%`. Нельзя округлять частично
...
Принятые 72,056,832 weights занимают 137.4375 MiB в BF16. После настоящей
Q2-g128 упаковки их расчётный payload составит 18.2534 MiB, экономия —
119.1841 MiB. Экономия VRAM появится только после packed runtime; fake-quant
обучение её не даёт.
...
В `layer 24` приняты 65.33203125% g128-групп `up_proj`, 2.34375%
...
|---|---:|---:|---:|
| v3 | 0.996659 | 1.000418 | 0.979919 (PyTorch) |
| v4 | 0.996659 | 0.996792 | 0.979505 (Transformers) |
...
1. **NLL budget.** Для условной цели `+5% NLL` и coverage `c` используем
   диагностическую линию `1 + 0.05*c`. При текущем `c=0.04188252` это
   `1.00209413`.
...
```text
wal2/checkpoints/wal-tat-block24_gate_after_recovery_s0011-candidate_only.pt
```

- размер: `304,659,052` bytes;
- SHA-256: `1552e033bebcff766da0dd7c4e20058f589a69e6970b8fdfc97206ef8af72d37`;

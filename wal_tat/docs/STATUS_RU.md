...
|---|---:|---:|
| Полные decoder blocks | 1 / 28 (`layer 27`) | 4 / 28 (`27`, `24`, `23`, `22`) |
| Полные крупные матрицы | 11 / 197 | 28 / 197 |
| Ternary weights | 76,673,024 (`4.456565%`) | 77,872,512 (`4.526285%`) |
| Q4-g128 weights | 0 | 64,314,368 (`3.738227%`) |
| Q8-g128 weights | 0 | 59,139,712 (`3.437454%`) |
| Все low-bit weights | 76,673,024 | 201,326,592 (`11.701966%`) |
| Осталось high precision | 1,643,778,048 | 1,519,124,480 (`88.298034%`) |
...
`layer 22` потребовал более дорогого fallback: `19.999949%` групп остались
Q4, `80.000051%` стали Q8, средняя точность блока `7.325002 bpw`. Sealed
audit-v28 прошёл с ratios `0.991651 / 0.993830 / 0.938898` и incremental
worst `1.002913 <= 1.005`. Это accepted deployable upper bound; блок нужно
позже обратно ужать Q8→Q4→Q2.
...
77,872,512 Q2-весов, 64,314,368 Q4-весов и 59,139,712 Q8-весов. Текущий
425,021,882-byte `.pt` хранит дублирующиеся int8 training arrays и не является
packed deploy-файлом. Расчётный packed payload этих low-bit весов —
108.634 MiB против 384 MiB в BF16; реальная экономия VRAM появится только
...
`model.layers.22` также полностью low-bit: 10,066,304 весов используют Q4 и
40,265,344 — Q8. BF16 в семи крупных матрицах блока нет.
...
`a66239a6afb965cc1847432292036f84c31fe850b91cdf1e06a48e6cd9751f7a`.

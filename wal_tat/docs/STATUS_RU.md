...
| Матрицы в `layer 24` | Q/K/V/O + 67.67578125% up + 2.5390625% gate + 5.76171875% down | 32.32421875% up + 97.4609375% gate + 94.23828125% down |
| Крупные матрицы, включая tied embedding/head | 11 / 197 | 186 |
| Крупные matrix weights | 72,474,624 / 1,720,451,072 | 1,647,976,448 |

Покрытие крупных matrix weights равно `4.212536%`. Нельзя округлять частично
...
Принятые 72,474,624 weights занимают 138.2344 MiB в BF16. После настоящей
Q2-g128 упаковки их расчётный payload составит 18.3593 MiB, экономия —
119.8751 MiB. Экономия VRAM появится только после packed runtime; fake-quant
обучение её не даёт.
...
`gate_proj` и 5.76171875% `down_proj`. Остальные 32.32421875% `up_proj`,
97.4609375% `gate_proj` и 94.23828125% `down_proj` пока остаются BF16.
...
|---|---:|---:|---:|
| v3 | 0.996861 | 1.000809 | 0.980576 (PyTorch) |
| v4 | 0.996861 | 0.997052 | 0.980289 (Transformers) |
...
1. **NLL budget.** Для условной цели `+5% NLL` и coverage `c` используем
   диагностическую линию `1 + 0.05*c`. При текущем `c=0.04212536` это
   `1.00210627`.
...
```text
wal2/checkpoints/wal-tat-block24_down_after_recovery_s0013-candidate_only.pt
```

- размер: `304,659,052` bytes;
- SHA-256: `82151728f6632112d139034a76bef4c58731913b5c297ca79f80d2b409d93d26`;
- содержание: полный ternary block 27, Q/K/V/O block 24, 67.67578125%
  `up_proj`, 2.5390625% `gate_proj` и 5.76171875% `down_proj`;
...
1. продолжить round-robin с `gate_proj +0.1953125%` атомом;

...
| Матрицы в `layer 24` | Q/K/V/O + 54.39453125% up + 0.9765625% gate + 2.24609375% down | 45.60546875% up + 99.0234375% gate + 97.75390625% down |
| Крупные матрицы, включая tied embedding/head | 11 / 197 | 186 |
| Крупные matrix weights | 70,164,480 / 1,720,451,072 | 1,650,286,592 |

Покрытие крупных matrix weights равно `4.078261%`. Нельзя округлять частично
...
Принятые 70,164,480 weights занимают 133.8281 MiB в BF16. После настоящей
Q2-g128 упаковки их расчётный payload составит 17.7740 MiB, экономия —
116.0541 MiB. Экономия VRAM появится только после packed runtime; fake-quant
обучение её не даёт.
...
`gate_proj` и 2.24609375% `down_proj`. Остальные 45.60546875% `up_proj`,
99.0234375% `gate_proj` и 97.75390625% `down_proj` пока остаются BF16.
...
|---|---:|---:|---:|
| v3 | 0.996633 | 1.000405 | 0.978772 (PyTorch) |
| v4 | 0.996633 | 0.996103 | 0.977332 (Transformers) |
...
1. **NLL budget.** Для условной цели `+5% NLL` и coverage `c` используем
   диагностическую линию `1 + 0.05*c`. При текущем `c=0.04078261` это
   `1.00203913`.
...
```text
wal2/checkpoints/wal-tat-block24_up_headroom_growth_s0008-candidate_only.pt
```

- размер: `304,658,813` bytes;
- SHA-256: `f9b43acae709e0dc20724e6063e41d0d4e646e76bec2c034c6478e8eab332261`;

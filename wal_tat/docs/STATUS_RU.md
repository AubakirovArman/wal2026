...
| Матрицы в `layer 24` | Q/K/V/O + 68.45703125% up + 2.734375% gate + 6.15234375% down | 31.54296875% up + 97.265625% gate + 93.84765625% down |
| Крупные матрицы, включая tied embedding/head | 11 / 197 | 186 |
| Крупные matrix weights | 72,646,656 / 1,720,451,072 | 1,647,804,416 |

Покрытие крупных matrix weights равно `4.222535%`. Нельзя округлять частично
...
Принятые 72,646,656 weights занимают 138.5625 MiB в BF16. После настоящей
Q2-g128 упаковки их расчётный payload составит 18.4028 MiB, экономия —
120.1597 MiB. Экономия VRAM появится только после packed runtime; fake-quant
обучение её не даёт.
...
В `layer 24` приняты 68.45703125% g128-групп `up_proj`, 2.734375%
`gate_proj` и 6.15234375% `down_proj`. Остальные 31.54296875% `up_proj`,
97.265625% `gate_proj` и 93.84765625% `down_proj` пока остаются BF16.
...
|---|---:|---:|---:|
| v3 | 0.996982 | 1.000850 | 0.980528 (PyTorch) |
| v4 | 0.996982 | 0.997187 | 0.980330 (Transformers) |
...
1. **NLL budget.** Для условной цели `+5% NLL` и coverage `c` используем
   диагностическую линию `1 + 0.05*c`. При текущем `c=0.04222535` это
   `1.00211127`.
...
```text
wal2/checkpoints/wal-tat-block24_down_after_recovery_s0014-candidate_only.pt
```

- размер: `304,659,052` bytes;
- SHA-256: `778a9fb8259a63e2ae09ab542883af67bd571230381e496f4257abca5348ed01`;
- содержание: полный ternary block 27, Q/K/V/O block 24, 68.45703125%
  `up_proj`, 2.734375% `gate_proj` и 6.15234375% `down_proj`;

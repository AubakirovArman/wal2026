...
| Матрицы в `layer 24` | Q/K/V/O + 31.93359375% `up_proj` | 68.06640625% up + down/gate |
| Крупные матрицы, включая tied embedding/head | 11 / 197 | 186 |
| Крупные matrix weights | 66,932,736 / 1,720,451,072 | 1,653,518,336 |

Покрытие крупных matrix weights равно `3.890418%`. Нельзя округлять частично
...
Принятые 66,932,736 weights занимают 127.6641 MiB в BF16. После настоящей
Q2-g128 упаковки их расчётный payload составит 16.9554 MiB, экономия —
110.7087 MiB. Экономия VRAM появится только после packed runtime; fake-quant
обучение её не даёт.
...
В `layer 24 up_proj` независимо приняты 31.93359375% g128-групп. Остальные
68.06640625% этой матрицы, а также `down_proj` и `gate_proj`, пока остаются
BF16.
...
|---|---:|---:|---:|
| v3 | 0.997572 | 1.001675 | 0.978439 (PyTorch) |
| v4 | 0.997572 | 0.998105 | 0.976555 (Transformers) |
...
1. **NLL budget.** Для условной цели `+5% NLL` и coverage `c` используем
   диагностическую линию `1 + 0.05*c`. При текущем `c=0.03890418` это
   `1.00194521`.
...
принятый coverage `up_proj` теперь 31.93359375%. Попытка добавить 12.5% за один
...
прошли fresh reload и оба holdout, подняв frontier до `31.93359375%`.

## Лучший воспроизводимый checkpoint

```text
wal2/checkpoints/wal-tat-block24_up_bf16_mlp_adaptive_31p93359375_v7-candidate_bf16_mlp.pt
```

- размер: `304,666,046` bytes;
- SHA-256: `cacc38beeb2be34d67931ccc8660c10038322c4f5c4099792a7394f9e23de9f0`;
- содержание: полный ternary block 27, Q/K/V/O block 24, 31.93359375% его

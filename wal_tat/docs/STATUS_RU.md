...
| Матрицы в `layer 24` | Q/K/V/O + 51.26953125% up + 0.9765625% gate + 2.24609375% down | 48.73046875% up + 99.0234375% gate + 97.75390625% down |
| Крупные матрицы, включая tied embedding/head | 11 / 197 | 186 |
| Крупные matrix weights | 69,771,264 / 1,720,451,072 | 1,650,679,808 |

Покрытие крупных matrix weights равно `4.055405%`. Нельзя округлять частично
...
Принятые 69,771,264 weights занимают 133.0781 MiB в BF16. После настоящей
Q2-g128 упаковки их расчётный payload составит 17.6744 MiB, экономия —
115.4037 MiB. Экономия VRAM появится только после packed runtime; fake-quant
обучение её не даёт.
...
В `layer 24` приняты 51.26953125% g128-групп `up_proj`, 0.9765625%
...
1. **NLL budget.** Для условной цели `+5% NLL` и coverage `c` используем
   диагностическую линию `1 + 0.05*c`. При текущем `c=0.04055405` это
   `1.00202770`.

...
| Матрицы в `layer 24` | Q/K/V/O + 63.76953125% up + 2.1484375% gate + 4.58984375% down | 36.23046875% up + 97.8515625% gate + 95.41015625% down |
| Крупные матрицы, включая tied embedding/head | 11 / 197 | 186 |
| Крупные matrix weights | 71,786,496 / 1,720,451,072 | 1,648,664,576 |

Покрытие крупных matrix weights равно `4.172539%`. Нельзя округлять частично
...
Принятые 71,786,496 weights занимают 136.9219 MiB в BF16. После настоящей
Q2-g128 упаковки их расчётный payload составит 18.1849 MiB, экономия —
118.7369 MiB. Экономия VRAM появится только после packed runtime; fake-quant
обучение её не даёт.
...
В `layer 24` приняты 63.76953125% g128-групп `up_proj`, 2.1484375%
...
|---|---:|---:|---:|
| v3 | 0.996529 | 1.000248 | 0.979831 (PyTorch) |
| v4 | 0.996529 | 0.996440 | 0.979109 (Transformers) |
...
1. **NLL budget.** Для условной цели `+5% NLL` и coverage `c` используем
   диагностическую линию `1 + 0.05*c`. При текущем `c=0.04172539` это
   `1.00208627`.
...
```text
wal2/checkpoints/wal-tat-block24_gate_after_recovery_s0010-candidate_only.pt
```

- размер: `304,659,052` bytes;
- SHA-256: `102cb13c049957271e1010959f23d21785062424cfc6ea5c7b6d7cb58fbbcdf9`;
...
1. продолжить round-robin с `up_proj +1.5625%` атомом;

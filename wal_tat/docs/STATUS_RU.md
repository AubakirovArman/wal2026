...
| Матрицы в `layer 24` | Q/K/V/O + 41.89453125% up + 0.1953125% gate + 0.68359375% down | 58.10546875% up + 99.8046875% gate + 99.31640625% down |
| Крупные матрицы, включая tied embedding/head | 11 / 197 | 186 |
| Крупные matrix weights | 68,296,704 / 1,720,451,072 | 1,652,154,368 |

Покрытие крупных matrix weights равно `3.969698%`. Нельзя округлять частично
...
Принятые 68,296,704 weights занимают 130.2656 MiB в BF16. После настоящей
Q2-g128 упаковки их расчётный payload составит 17.3009 MiB, экономия —
112.9647 MiB. Экономия VRAM появится только после packed runtime; fake-quant
обучение её не даёт.
...
`gate_proj` и 0.68359375% `down_proj`. Остальные 58.10546875% `up_proj`,
...
1. **NLL budget.** Для условной цели `+5% NLL` и coverage `c` используем
   диагностическую линию `1 + 0.05*c`. При текущем `c=0.03969698` это
   `1.00198485`.

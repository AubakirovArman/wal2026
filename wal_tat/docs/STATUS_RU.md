...
| Матрицы в `layer 24` | Q/K/V/O + 68.45703125% up + 2.734375% gate + 5.76171875% down | 31.54296875% up + 97.265625% gate + 94.23828125% down |
| Крупные матрицы, включая tied embedding/head | 11 / 197 | 186 |
| Крупные matrix weights | 72,597,504 / 1,720,451,072 | 1,647,853,568 |

Покрытие крупных matrix weights равно `4.219679%`. Нельзя округлять частично
...
Принятые 72,597,504 weights занимают 138.4688 MiB в BF16. После настоящей
Q2-g128 упаковки их расчётный payload составит 18.3904 MiB, экономия —
120.0784 MiB. Экономия VRAM появится только после packed runtime; fake-quant
обучение её не даёт.
...
`gate_proj` и 5.76171875% `down_proj`. Остальные 31.54296875% `up_proj`,
97.265625% `gate_proj` и 94.23828125% `down_proj` пока остаются BF16.
...
|---|---:|---:|---:|
| v3 | 0.996918 | 1.000762 | 0.980530 (PyTorch) |
| v4 | 0.996918 | 0.997124 | 0.980285 (Transformers) |
...
1. **NLL budget.** Для условной цели `+5% NLL` и coverage `c` используем
   диагностическую линию `1 + 0.05*c`. При текущем `c=0.04219679` это
   `1.00210984`.
...
```text
wal2/checkpoints/wal-tat-block24_up_headroom_growth_s0019-candidate_only.pt
```

- размер: `304,658,813` bytes;
- SHA-256: `34933ae89de07b6cc792e8b1a7bb2ee2555dc2dc0c88201ae1b6c239bbb91bdc`;
- содержание: полный ternary block 27, Q/K/V/O block 24, 68.45703125%
  `up_proj`, 2.734375% `gate_proj` и 5.76171875% `down_proj`;

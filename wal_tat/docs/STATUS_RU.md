...
| Матрицы в `layer 24` | Q/K/V/O + 70.80078125% up + 3.3203125% gate + 6.93359375% down | 29.19921875% up + 96.6796875% gate + 93.06640625% down |
| Крупные матрицы, включая tied embedding/head | 11 / 197 | 186 |
| Крупные matrix weights | 73,113,600 / 1,720,451,072 | 1,647,337,472 |

Покрытие крупных matrix weights равно `4.249676%`. Нельзя округлять частично
...
Принятые 73,113,600 weights занимают 139.453125 MiB в BF16. После настоящей
Q2-g128 упаковки их расчётный payload составит 18.5211 MiB, экономия —
120.9320 MiB. Экономия VRAM появится только после packed runtime; fake-quant
обучение её не даёт.
...
В `layer 24` приняты 70.80078125% g128-групп `up_proj`, 3.3203125%
`gate_proj` и 6.93359375% `down_proj`. Остальные 29.19921875% `up_proj`,
96.6796875% `gate_proj` и 93.06640625% `down_proj` пока остаются BF16.
...
|---|---:|---:|---:|
| v3 | 0.996668 | 1.000784 | 0.980086 (PyTorch) |
| v4 | 0.996668 | 0.997090 | 0.980155 (Transformers) |
...
1. **NLL budget.** Для условной цели `+5% NLL` и coverage `c` используем
   диагностическую линию `1 + 0.05*c`. При текущем `c=0.04249676` это
   `1.00212484`.
...
```text
wal2/checkpoints/wal-tat-block24_up_headroom_growth_s0022-candidate_only.pt
```

- размер: `304,658,813` bytes;
- SHA-256: `09effb690206d038327e4d63ee37fb42bf8b7a4f11e6f11b1e591d9b2a197cdf`;
- содержание: полный ternary block 27, Q/K/V/O block 24, 70.80078125%
  `up_proj`, 3.3203125% `gate_proj` и 6.93359375% `down_proj`;

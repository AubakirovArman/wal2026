...
| Матрицы в `layer 24` | Q/K/V/O + 74.31640625% up + 4.4921875% gate + 8.88671875% down | 25.68359375% up + 95.5078125% gate + 91.11328125% down |
| Крупные матрицы, включая tied embedding/head | 11 / 197 | 186 |
| Крупные matrix weights | 73,949,184 / 1,720,451,072 | 1,646,501,888 |

Покрытие крупных matrix weights равно `4.298244%`. Нельзя округлять частично
...
Принятые 73,949,184 weights занимают 141.0469 MiB в BF16. После настоящей
Q2-g128 упаковки их расчётный payload составит 18.7328 MiB, экономия —
122.3141 MiB. Экономия VRAM появится только после packed runtime; fake-quant
обучение её не даёт.
...
В `layer 24` приняты 74.31640625% g128-групп `up_proj`, 4.4921875%
`gate_proj` и 8.88671875% `down_proj`. Остальные 25.68359375% `up_proj`,
95.5078125% `gate_proj` и 91.11328125% `down_proj` пока остаются BF16.
...
|---|---:|---:|---:|
| v3 | 0.996738 | 1.001076 | 0.980458 (PyTorch) |
| v4 | 0.996738 | 0.997347 | 0.980923 (Transformers) |
...
1. **NLL budget.** Для условной цели `+5% NLL` и coverage `c` используем
   диагностическую линию `1 + 0.05*c`. При текущем `c=0.04298244` это
   `1.00214912`.
...
```text
wal2/checkpoints/wal-tat-block24_up_headroom_growth_s0032-candidate_only.pt
```

- размер: `304,658,813` bytes;
- SHA-256: `b5228f1ce45f292030042f1b2d504703b89404bf434b7c20780f73c1d55bb20f`;
- содержание: полный ternary block 27, Q/K/V/O block 24, 74.31640625%
  `up_proj`, 4.4921875% `gate_proj` и 8.88671875% `down_proj`;
...
1. продолжить минимальным `down_proj +0.09765625%` либо сделать rollback;

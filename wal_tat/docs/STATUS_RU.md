...
| Ternary weights | 76,673,024 (`4.456565%`) | 79,426,432 (`4.616605%`) |
| Q4-g128 weights | 0 | 72,806,656 (`4.231835%`) |
| Q8-g128 weights | 0 | 99,425,152 (`5.779017%`) |
...
`layer 25` полностью low-bit: после первого Q8→Q4 reverse-compression шага
он содержит `19.960022%` Q4 и `80.039978%` Q8 при `7.326599 bpw`.
`6,019,712` весов стали Q4 без изменения coverage. Fresh development дал
`0.994980 / 1.001201 / 1.019962`; sealed audit-v32 —
`0.993773 / 0.943618 / 0.968501`. Incremental worst равен `1.003159` против
immutable `s0053` и `1.000108` против принятого parent, оба ниже `1.005`.
...
79,426,432 Q2-весов, 72,806,656 Q4-весов и 99,425,152 Q8-весов. Текущий
579,579,257-byte `.pt` хранит дублирующиеся int8 training arrays и не является
packed deploy-файлом. Расчётный packed payload этих low-bit весов —
152.223 MiB против 480 MiB в BF16; реальная экономия VRAM появится только
...
`model.layers.25` полностью low-bit: 10,046,208 весов используют Q4 и
40,285,440 — Q8. BF16 в семи крупных матрицах блока нет.
...
Он прошёл fresh development и sealed audit-v32. Ни Q4/Q8-веса, ни mixed

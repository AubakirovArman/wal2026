...
| Матрицы в `layer 24` | Q/K/V/O + 51.26953125% up + 0.78125% gate + 1.85546875% down | 48.73046875% up + 99.21875% gate + 98.14453125% down |
| Крупные матрицы, включая tied embedding/head | 11 / 197 | 186 |
| Крупные matrix weights | 69,697,536 / 1,720,451,072 | 1,650,753,536 |

Покрытие крупных matrix weights равно `4.051120%`. Нельзя округлять частично
...
Принятые 69,697,536 weights занимают 132.9375 MiB в BF16. После настоящей
Q2-g128 упаковки их расчётный payload составит 17.6558 MiB, экономия —
115.2817 MiB. Экономия VRAM появится только после packed runtime; fake-quant
обучение её не даёт.
...
`gate_proj` и 1.85546875% `down_proj`. Остальные 48.73046875% `up_proj`,
...
|---|---:|---:|---:|
| v3 | 0.996993 | 1.000962 | 0.979244 (PyTorch) |
| v4 | 0.996993 | 0.996790 | 0.977836 (Transformers) |
...
1. **NLL budget.** Для условной цели `+5% NLL` и coverage `c` используем
   диагностическую линию `1 + 0.05*c`. При текущем `c=0.04051120` это
   `1.00202556`.
...
```text
wal2/checkpoints/wal-tat-block24_up_headroom_growth_s0007-candidate_bf16_mlp.pt
```

- размер: `304,659,641` bytes;
- SHA-256: `6a1975f48cbcf50ae3673f4b2e626317121cdcffdba02f2ea2658eb1241befd0`;

#!/usr/bin/env python3
"""
Fix missing 'pass' fields in result JSONs and regenerate book/diary.
If result JSON exists and has data, set pass=true if not explicitly false.
"""
...
def generate_book(mod_num, title, result_data, py_path):
    ...
    # Smart status: if JSON exists and is not explicitly false → PASS
...
def main():
    ...
    for py_path in sorted(glob.glob(f"{RESULT_DIR}/m*.py")):
        ...
        result_data, result_path = get_result(py_path)
        
        # Fix missing pass: if JSON exists and pass not set, add pass=true
    ...
    lines = []
    lines.append("\n\n---\n")
    lines.append("# ПОЛНЫЙ ДЕТАЛЬНЫЙ ОТЧЁТ M386–M620 (исправленный)\n")
